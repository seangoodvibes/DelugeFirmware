...
void reset_transitions_for_session() {
	uiTimerManager.unsetTimer(TimerName::MATRIX_DRIVER);
	auto& panel = state();
	panel.zoomingIn = false;
	panel.zoomMagnitude = 0;
	std::fill_n(panel.zoomPinSquare, kDisplayHeight, 0);
	std::fill_n(panel.transitionTakingPlaceOnRow, kDisplayHeight, false);
	panel.explodeAnimationDirection = 0;
	panel.explodeAnimationTargetUI = nullptr;
	panel.numAnimatedRows = 0;
	panel.greyProportion = 0;
	panel.greyoutChangeDirection = 0;
	panel.greyoutChangeStartTime = 0;
	panel.greyoutCols = panel.greyoutRows = 0;
	panel.transitionLength = panel.transitionStartTime = 0;
	panel.horizontal = {};
	panel.vertical = {};
	panel.morphKeyboardSidebar = false;
	std::fill_n(panel.slowFlashSquares, kDisplayHeight, 255);
	std::fill_n(panel.slowFlashColours, kDisplayHeight, 0);
	panel.needToSendOutMainPadColours = false;
	panel.needToSendOutSidebarColours = false;
}
