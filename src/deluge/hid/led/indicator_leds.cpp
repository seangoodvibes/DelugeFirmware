...
void reset_blinking_for_session() {
	uiTimerManager.unsetTimer(TimerName::LED_BLINK);
	uiTimerManager.unsetTimer(TimerName::LED_BLINK_TYPE_1);
	uiTimerManager.unsetTimer(TimerName::LEVEL_INDICATOR_BLINK);
	uiTimerManager.unsetTimer(TimerName::METER_INDICATOR_BLINK);
	auto& panel = panel_state();
	panel.ledBlinkers = {};
	panel.ledBlinkState = {};
	panel.whichLevelIndicatorBlinking = 0;
	panel.levelIndicatorBlinkOn = false;
	panel.levelIndicatorBlinksLeft = 0;
	panel.levelIndicatorBipolar = false;
	panel.whichKnobMetering = 0;
}
