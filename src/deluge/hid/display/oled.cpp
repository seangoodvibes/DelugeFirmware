...
void OLED::reset_layers_for_session() {
	// Cancel retained blink/scroll state before startup services UI timers.
	clearMainImage();
}
...
void OLED::clear_console_for_session() {
	panel_state().numConsoleItems = 0;
	panel_state().consoleMinX = -1;
	panel_state().consoleMaxX = 0;
	uiTimerManager.unsetTimer(TimerName::OLED_CONSOLE);
	markChanged();
}
