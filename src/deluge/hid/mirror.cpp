...
bool preparing_remote_snapshot = false;
bool remote_initialization_pending = false;
struct preparation_guard {
	preparation_guard() { preparing_remote_snapshot = true; }
...
void stop(const char* reason) {
	...
	if (active_session_mode == protocol::session_mode::independent) {
		// Release callbacks can rearm UI timers. Cancel the entire Remote bank
		// afterward, while hardware timers remain owned by Local.
		for (int timer_index = 0; timer_index < static_cast<int>(TimerName::NUM_TIMERS); ++timer_index) {
			const auto timer = static_cast<TimerName>(timer_index);
			if (UITimerState::owner(timer) == deluge::gui::ui_session::Id::Remote)
				uiTimerManager.unsetTimer(timer);
		}
		...
		auto& navigation = deluge::gui::ui_session::navigation.for_owner(deluge::gui::ui_session::Id::Remote);
		navigation.hierarchy.fill(nullptr);
		navigation.depth = 0;
		navigation.last_before_nullifying = nullptr;
		navigation.mode = 0;
		navigation.main_rows_dirty = navigation.side_rows_dirty = 0;
		navigation.oled_dirty = true;
		navigation.structural_refresh.request();
		// Preserve the rendering guard if teardown occurred inside a render;
		// the suspended renderer still owns responsibility for clearing it.
	}
...
bool panel_byte(uint8_t byte) {
	...
	if (position == length) {
		if (allowed && !is_client()) {
			...
			// Initial preparation may render repeatedly while acceptance is
			// blocked. Its snapshot is rebuilt below; do not accumulate unsent
			// animation/render commands from these attempts.
			if (state == State::Host && !preparing_remote_snapshot
			    && (owner == input_owner() || (!remote && shared_panel_setting(command))))
...
void initialize_remote_root() {
	if (!remote_initialization_pending || preparing_remote_snapshot || failed || sdRoutineLock
	    || AudioEngine::audioRoutineLocked)
		return;
	if (state != State::Host || active_session_mode != protocol::session_mode::independent)
		return;
	deluge::gui::ui_session::Scope owner(deluge::gui::ui_session::Id::Remote);
	auto& navigation = deluge::gui::ui_session::navigation.active();
	...
	auto* root = &session_view_for_session();
	// Lazy construction may service callbacks. Validate before publishing the
	// root into navigation, not only after installing it.
	if (failed || state != State::Host || currentSong != source_song || !session_live()
	    || deluge::gui::ui_session::current() != deluge::gui::ui_session::Id::Remote || navigation.depth) {
		failed = true;
		return;
	}
	// This view instance survives disconnect, but its pulse timer does not.
	// Clear the cached clip/active flag before focusRegained decides to rearm.
	PadLEDs::reset_transitions_for_session();
	indicator_leds::reset_blinking_for_session();
	Buttons::reset_for_session_startup();
	root->reset_for_remote_startup();
	navigation.mode = 0;
	navigation.last_before_nullifying = nullptr;
	setRootUILowLevel(root);
	auto installed_root_valid = [&] {
		return !failed && state == State::Host && currentSong == source_song && session_live()
	...
	if (::display->haveOLED())
		display::OLED::clear_console_for_session();
	if (!root->opened() || !installed_root_valid() || !remote_ui_ready()) {
		failed = true;
	}
}
...
bool prepare_remote_snapshot() {
	if (active_session_mode != protocol::session_mode::independent)
		return true;
	if (!remote_ui_ready() || preparing_remote_snapshot)
		return false;
	preparation_guard guard;
	deluge::gui::ui_session::Scope owner(deluge::gui::ui_session::Id::Remote);
	auto* const source_song = currentSong;
	...
	if (::display->haveOLED()
	    && (display::OLED::remote_frame_revision() == previous_frame_revision
	        || !display::OLED::copy_remote_frame({oled_snapshot, oled_size})))
		return false;
...
void routine() {
	...
	if (state != State::Idle) {
		...
		if (failed || !session_live() || (is_client() && back_since && now - back_since > 2.0)) {
			...
			if (state == State::Host && snapshot_pending && !failed && prepare_remote_snapshot() && send(Op::Accept)
			    && !failed) {
