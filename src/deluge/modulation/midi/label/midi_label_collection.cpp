...
void MIDILabelCollection::writeToFile(Serializer& writer) {
	if (labels.getNumElements()) {

		writer.writeOpeningTag("midiLabels");

		for (int32_t i = 0; i < labels.getNumElements(); i++) {
			MIDILabel* midiLabel = labels.getElement(i);
			int32_t cc = midiLabel->cc;
		}

		writer.writeClosingTag("midiLabels");
	}
}
