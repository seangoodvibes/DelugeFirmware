...
// USB queue constants.
// USB-MIDI event packets are stored little-endian in a uint32_t:
...
constexpr int32_t k_usb_cc_message_budget_per_transfer = 8;

// DIN serial pacing constants.
// DIN pacing uses Q8 fixed-point bytes so fractional byte budget can accrue
// smoothly between audio callbacks.
constexpr int32_t k_serial_budget_fraction_bits = 8;
constexpr int32_t k_serial_budget_fraction_scale = 1 << k_serial_budget_fraction_bits;
// DIN link throughput in Q8 fixed-point bytes/second (31.25 kbps ~= 3125 bytes/s).
constexpr int32_t k_serial_bytes_per_second_Q8 = 3125 * k_serial_budget_fraction_scale;
// Maximum accumulated send budget (Q8 bytes) allowed for one burst after idle time.
constexpr int32_t k_serial_queue_budget_max_Q8 = MIDI_TX_BUFFER_SIZE * k_serial_budget_fraction_scale;
...
bool MIDIQueueManagerUSB::consume_queued_messages(uint8_t* data_sending_now, uint8_t& num_bytes_sending_now,
	...
	int32_t cc_budget_messages_remaining = k_usb_cc_message_budget_per_transfer;
	...
	for (i = 0; i < to_send; i++) {
		...
		USBSendContext context{message, cc_budget_messages_remaining};
		bool popped = false;
		...
		for (uint8_t lane = static_cast<uint8_t>(QUEUE_PRIORITY_CLOCK);
			...
			if (pop_lane(priority, context)) {
				// Normal priority pop succeeded; this transfer slot is filled.
				popped = true;
				break;
			}
		}
...
QueuePriority MIDIQueueManagerUSB::classify_packed_usb_priority(uint32_t packed) {
	// CIN is the low nibble of byte 0 and identifies USB-MIDI event framing.
...
MIDIQueueManager::PriorityLaneTraversalResult MIDIQueueManagerUSB::handle_cc_lane(QueuePriority priority,
	...
	auto cc_result = MIDIQueueManager::try_pop_scheduled_cc(is_packed_channel_cc(head_message),
	                                                        context.cc_budget_messages_remaining > 0, pop_scheduled_cc,
	                                                        context.message_out);
	if (cc_result == MIDIQueueManager::CCScheduledPopResult::Popped) {
		// Count only scheduled CC pops against the per-transfer CC budget.
		context.cc_budget_messages_remaining--;
	...
	// Budget exhaustion or pop failure means this lane should not emit now.
...
void MIDIQueueManagerDIN::reset_serial_state(uint32_t now_sample_timer) {
	// Start budget accrual from the caller's current audio sample timestamp.
	serial_budget_last_update_ = now_sample_timer;
	// Do not start with preloaded send credit.
	serial_budget_Q8_ = 0;
}
...
void MIDIQueueManagerDIN::consume_queued_messages(uint32_t now_sample_timer) {
	...
	// Apply DIN pacing before deciding this iteration's send allowance. The
	// token bucket uses Q8 bytes so fractional bytes accrue smoothly between
	// audio callbacks.
	...
	int32_t send_allowance_bytes = serial_budget_Q8_ >> k_serial_budget_fraction_bits;
	...
	while (uart_space > 0 && send_allowance_bytes > 0) {
		...
		for (uint8_t lane = static_cast<uint8_t>(k_clock_idx); lane <= static_cast<uint8_t>(k_cc_idx); lane++) {
			...
			if (priority == QUEUE_PRIORITY_CC) {
				...
				if (cc_result == MIDIQueueManager::PriorityLaneTraversalResult::Abort) {
					// The CC lane is blocked, malformed, or over its staging budget.
					break;
				}
				if (cc_result == MIDIQueueManager::PriorityLaneTraversalResult::SkipLane) {
			...
			if (pop_lane(priority, context)) {
				// Normal lane pop succeeded; bytes_to_send now contains one message.
				popped = true;
				break;
			}
		}
		...
		// Reconstruct how many bytes were popped so budgets can be debited correctly.
	...
	if (sent > 0) {
		...
		serial_budget_Q8_ = std::max<int32_t>(0, serial_budget_Q8_ - sent * k_serial_budget_fraction_scale);
	}
}
...
bool MIDIQueueManagerDIN::pop_lane(QueuePriority priority, DINSendContext& context) {
	if (priority == QUEUE_PRIORITY_CLOCK) {
		...
		if (context.budget_bytes < k_midi_realtime_message_bytes || context.uart_space < k_midi_realtime_message_bytes
		    || context.max_len < k_midi_realtime_message_bytes) {
	...
	// message is available and fits the current budget/UART/output limits.
...
MIDIQueueManager::PriorityLaneTraversalResult MIDIQueueManagerDIN::handle_cc_lane(QueuePriority priority,
	...
	auto pop_scheduled_cc = [this](uint8_t* out_bytes, int32_t budget_bytes, int32_t uart_space, int32_t max_len,
	                               QueuePriority& popped_priority) {
		// Delegate scheduled removal to the DIN-specific complete-message popper.
		return pop_next_scheduled_cc_message(out_bytes, budget_bytes, uart_space, max_len, popped_priority);
	};
	auto cc_result = MIDIQueueManager::try_pop_scheduled_cc(
	    head_is_cc, context.cc_uart_budget >= MIDIQueueManager::k_channel_cc_message_length, pop_scheduled_cc,
	...
	// Budget exhaustion or pop failure means the CC lane should not emit now.
	return MIDIQueueManager::PriorityLaneTraversalResult::Abort;
}

bool MIDIQueueManagerDIN::pop_next_scheduled_cc_message(uint8_t* out_bytes, int32_t budget_bytes, int32_t uart_space,
                                                        int32_t max_len, QueuePriority& popped_priority) {
	if (budget_bytes < MIDIQueueManager::k_channel_cc_message_length
