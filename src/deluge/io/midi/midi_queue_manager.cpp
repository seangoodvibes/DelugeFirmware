...
// Allowed UI/storage range for decimation rates, in milliseconds.
constexpr uint8_t kDecimationRateMsDefault = 1;
constexpr uint8_t kDecimationRateMsMin = 1;
constexpr uint8_t kDecimationRateMsMax = 20;
} // namespace

MidiQueueManager midiQueueManager{};

MidiQueueManager::MidiQueueManager() {
	setCcDecimationRateMs(kDecimationRateMsDefault);
	setExpressionDecimationRateMs(kDecimationRateMsDefault);
	resetMessageDecimationState();
}

/// Classifies an outgoing MIDI message into shared queue priorities.
MidiQueueManager::QueuePriority MidiQueueManager::classifyMessage(MIDIMessage message) {
	if (message.isSystemMessage()) {
		// Keep transport / realtime bytes at the highest priority lane.
		return QueuePriority::CLOCK;
	}
...
/// Maps shared queue priorities to USB ring-lane priority constants.
uint8_t MidiQueueManager::toUsbPriority(QueuePriority priority) {
	// Map shared queue priorities onto existing USB queue lane constants.
	switch (priority) {
	case QueuePriority::CLOCK:
		return MIDI_SEND_PRIORITY_CLOCK;
	case QueuePriority::NOTES:
		return MIDI_SEND_PRIORITY_NOTES;
	case QueuePriority::EXPRESSION:
		return MIDI_SEND_PRIORITY_EXPRESSION;
	case QueuePriority::CC:
		return MIDI_SEND_PRIORITY_CC;
	case QueuePriority::SYSEX:
		return MIDI_SEND_PRIORITY_SYSEX;
	case QueuePriority::COUNT:
		return MIDI_SEND_PRIORITY_CC;
	}
...
uint16_t MidiQueueManager::usbQueueCount(USBMidiSendQueueStorage const* storage, uint8_t priority) {
	// Monotonic write/read counters: occupancy is their difference for each lane.
	return static_cast<uint16_t>(storage->ringBufWriteIdx[priority] - storage->ringBufReadIdx[priority]);
}
...
uint32_t MidiQueueManager::usbTotalQueuedMessages(USBMidiSendQueueStorage const* storage) {
	...
	uint32_t queued = 0;
	for (uint8_t p = 0; p < MIDI_SEND_PRIORITY_COUNT; p++) {
		queued += usbQueueCount(storage, p);
	}
	return queued;
}

/// Returns true if any USB lane above `priority` currently has queued packets.
bool MidiQueueManager::usbAnyHigherPriorityHasData(USBMidiSendQueueStorage const* storage, uint8_t priority) {
	// Lower numeric value means higher queue priority.
	for (uint8_t p = 0; p < priority; p++) {
		if (usbQueueCount(storage, p)) {
			return true;
		}
	}
	return false;
...
bool MidiQueueManager::usbPopPriorityMessage(USBMidiSendQueueStorage* storage, uint32_t& messageOut) {
	...
	for (uint8_t p = MIDI_SEND_PRIORITY_CLOCK; p <= MIDI_SEND_PRIORITY_EXPRESSION; p++) {
		if (usbQueueCount(storage, p)) {
	...
	if (!usbAnyHigherPriorityHasData(storage, MIDI_SEND_PRIORITY_CC) && usbQueueCount(storage, MIDI_SEND_PRIORITY_CC)) {
		messageOut = storage->sendDataRingBuf[MIDI_SEND_PRIORITY_CC]
		                                     [storage->ringBufReadIdx[MIDI_SEND_PRIORITY_CC] & MIDI_SEND_RING_MASK];
		storage->ringBufReadIdx[MIDI_SEND_PRIORITY_CC]++;
		return true;
	}

	// SysEx is always last and only dequeued when all higher lanes are clear.
	if (!usbAnyHigherPriorityHasData(storage, MIDI_SEND_PRIORITY_SYSEX)
	    && usbQueueCount(storage, MIDI_SEND_PRIORITY_SYSEX)) {
		messageOut = storage->sendDataRingBuf[MIDI_SEND_PRIORITY_SYSEX]
		                                     [storage->ringBufReadIdx[MIDI_SEND_PRIORITY_SYSEX] & MIDI_SEND_RING_MASK];
		storage->ringBufReadIdx[MIDI_SEND_PRIORITY_SYSEX]++;
		return true;
	}
...
void MidiQueueManager::usbPushPriorityMessage(USBMidiSendQueueStorage* storage, uint8_t priority, uint32_t message) {
	// Power-of-two mask wraps index without modulo cost.
	storage->sendDataRingBuf[priority][storage->ringBufWriteIdx[priority] & MIDI_SEND_RING_MASK] = message;
	storage->ringBufWriteIdx[priority]++;
}
...
bool MidiQueueManager::enqueueSerialBytes(QueuePriority priority, uint8_t const* bytes, int32_t len) {
	SerialByteQueue& queue = serialPriorityQueues_[static_cast<size_t>(priority)];
	if (len <= 0) {
		return true;
	}
	if (queue.space() < len) {
...
bool MidiQueueManager::hasHigherPriorityDataThan(QueuePriority priority) const {
	...
	for (size_t i = 0; i < static_cast<size_t>(priority); i++) {
...
int32_t MidiQueueManager::popNextPrioritizedBytes(uint8_t* outBytes, int32_t maxLen, int32_t budgetBytes,
	...
	constexpr size_t kClockIdx = static_cast<size_t>(QueuePriority::CLOCK);
	constexpr size_t kNotesIdx = static_cast<size_t>(QueuePriority::NOTES);
	constexpr size_t kExpressionIdx = static_cast<size_t>(QueuePriority::EXPRESSION);
	constexpr size_t kCcIdx = static_cast<size_t>(QueuePriority::CC);
	constexpr size_t kSysexIdx = static_cast<size_t>(QueuePriority::SYSEX);
	...
	if (!hasHigherPriorityDataThan(QueuePriority::CC)) {
	...
	if (!hasHigherPriorityDataThan(QueuePriority::SYSEX)) {
...
void MidiQueueManager::enqueueSerialSysex(uint8_t const* data, int32_t len) {
	...
	enqueueSerialBytes(QueuePriority::SYSEX, data, len);
}
...
/// Sets CC-message decimation interval and caches the corresponding sample threshold.
void MidiQueueManager::setCcDecimationRateMs(uint8_t rateMs) {
	// Clamp to UI/storage-supported range so runtime behavior stays bounded.
	rateMs = std::clamp(rateMs, kDecimationRateMsMin, kDecimationRateMsMax);
	ccDecimationRateMs_ = rateMs;
	// Convert milliseconds to sample-count threshold used by hot-path comparisons.
	ccDecimationMinSamples_ = (static_cast<uint32_t>(ccDecimationRateMs_) * kSampleRate) / 1000;
	if (!ccDecimationMinSamples_) {
		// Never allow a zero threshold, otherwise elapsed-time checks become no-ops.
		ccDecimationMinSamples_ = 1;
	}
}

/// Sets expression-message decimation interval and caches the corresponding sample threshold.
void MidiQueueManager::setExpressionDecimationRateMs(uint8_t rateMs) {
	// Clamp to UI/storage-supported range so runtime behavior stays bounded.
	rateMs = std::clamp(rateMs, kDecimationRateMsMin, kDecimationRateMsMax);
	expressionDecimationRateMs_ = rateMs;
	// Convert milliseconds to sample-count threshold used by hot-path comparisons.
	expressionDecimationMinSamples_ = (static_cast<uint32_t>(expressionDecimationRateMs_) * kSampleRate) / 1000;
	if (!expressionDecimationMinSamples_) {
		// Never allow a zero threshold, otherwise elapsed-time checks become no-ops.
		expressionDecimationMinSamples_ = 1;
	}
}

/// Resets cached decimation history for all channels/controllers.
void MidiQueueManager::resetMessageDecimationState() {
	for (size_t ch = 0; ch < 16; ch++) {
		// Sentinels guarantee that the first outgoing value per channel is accepted
		// (it cannot match a plausible MIDI payload value).
		lastValueMidiPB_[ch] = 0xFFFF;
		lastValueMidiAT_[ch] = 0xFF;
		// Zero timestamps mark "no prior send" for elapsed-time comparisons.
		lastMomentMidiPB_[ch] = 0;
		lastMomentMidiAT_[ch] = 0;

		for (size_t msg = 0; msg < 128; msg++) {
			// Per-controller/per-note sentinels for duplicate suppression caches.
			lastValueMidiCC_[ch][msg] = 0xFF;
			lastMomentMidiCC_[ch][msg] = 0;
			lastValueMidiPP_[ch][msg] = 0xFF;
			lastMomentMidiPP_[ch][msg] = 0;
		}
	}
}

/// Applies duplicate suppression and timing-based throttling to outgoing expression-like MIDI messages.
bool MidiQueueManager::shouldSendAfterDecimation(MIDIMessage message) {
	// This decimation strategy is inspired by the LinnStrument firmware's outgoing
	// MIDI traffic shaping: prioritize playability by reducing high-rate duplicate
	// expression traffic while preserving timing-critical events.
	if (message.isSystemMessage()) {
		// Realtime/system traffic (clock, start/stop, etc.) must not be decimated.
		return true;
	}

	// MIDI channels are encoded in the low nibble; normalize to [0, 15].
	uint8_t channel = message.channel & 0x0F;
	uint32_t now = AudioEngine::audioSampleTimer;

	switch (message.statusType) {
	case 0x0B: {
		// Control Change: suppress exact duplicates, and rate-limit non-zero
		// continuous controls unless they are channel-mode or sustain messages.
		uint8_t cc = message.data1 & 0x7F;
		uint8_t value = message.data2 & 0x7F;

		// Duplicate value for same channel/controller: drop immediately.
		if (lastValueMidiCC_[channel][cc] == value) {
			return false;
		}

		// Keep channel-mode controls (120-127), sustain (64), and zero values
		// responsive by bypassing interval gating.
		bool isChannelMode = cc >= 120;
		bool isSustain = cc == 64;
		if (!isChannelMode && !isSustain && value != 0) {
			uint32_t elapsed = now - lastMomentMidiCC_[channel][cc];
			if (elapsed <= ccDecimationMinSamples_) {
				// Too soon since prior sent value for this CC lane.
				return false;
			}
		}

		// Accept and cache latest value/timestamp.
		lastValueMidiCC_[channel][cc] = value;
		lastMomentMidiCC_[channel][cc] = now;
		return true;
	}

	case 0x0E: {
		// Pitch Bend (14-bit): suppress duplicates and rate-limit movement updates,
		// but allow center return (0x2000) through immediately.
		uint16_t bend = static_cast<uint16_t>((message.data2 & 0x7F) << 7) | (message.data1 & 0x7F);
		// Duplicate bend value on this channel: drop.
		if (lastValueMidiPB_[channel] == bend) {
			return false;
		}

		// Non-center bend changes are subject to expression decimation interval.
		if (bend != 0x2000) {
			uint32_t elapsed = now - lastMomentMidiPB_[channel];
			if (elapsed <= expressionDecimationMinSamples_) {
				// Too soon since prior pitch-bend transmission.
				return false;
			}
		}

		// Accept and cache latest value/timestamp.
		lastValueMidiPB_[channel] = bend;
		lastMomentMidiPB_[channel] = now;
		return true;
	}

	case 0x0D: {
		// Channel Aftertouch: suppress duplicates and decimate non-zero pressure
		// updates to avoid flooding while preserving release responsiveness.
		uint8_t value = message.data1 & 0x7F;
		// Duplicate pressure value on this channel: drop.
		if (lastValueMidiAT_[channel] == value) {
			return false;
		}

		// Non-zero pressure changes are subject to expression interval gating.
		if (value != 0) {
			uint32_t elapsed = now - lastMomentMidiAT_[channel];
			if (elapsed <= expressionDecimationMinSamples_) {
				// Too soon since prior channel-aftertouch transmission.
				return false;
			}
		}

		// Accept and cache latest value/timestamp.
		lastValueMidiAT_[channel] = value;
		lastMomentMidiAT_[channel] = now;
		return true;
	}

	case 0x0A: {
		// Polyphonic Aftertouch: track per-note values, suppress duplicates, and
		// decimate non-zero pressure updates per channel/note lane.
		uint8_t note = message.data1 & 0x7F;
		uint8_t value = message.data2 & 0x7F;
		// Duplicate per-note pressure value: drop.
		if (lastValueMidiPP_[channel][note] == value) {
			return false;
		}

		// Non-zero pressure changes are subject to expression interval gating.
		if (value != 0) {
			uint32_t elapsed = now - lastMomentMidiPP_[channel][note];
			if (elapsed <= expressionDecimationMinSamples_) {
				// Too soon since prior poly-aftertouch transmission for this note.
				return false;
			}
		}

		// Accept and cache latest value/timestamp.
		lastValueMidiPP_[channel][note] = value;
		lastMomentMidiPP_[channel][note] = now;
		return true;
	}

	default:
		// Unhandled channel voice messages are forwarded unmodified.
		return true;
	}
}
