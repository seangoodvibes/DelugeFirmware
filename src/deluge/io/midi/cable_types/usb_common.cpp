...
uint8_t classifyUSBPriority(MIDIMessage message) {
	// Reuse shared queue policy so USB and serial paths classify traffic identically.
	return MidiQueueManager::toUsbPriority(MidiQueueManager::classifyMessage(message));
}
} // namespace
...
void MIDICableUSB::sendMessage(MIDIMessage message) {
	...
	uint8_t priority = classifyUSBPriority(message);
...
void MIDICableUSB::sendSysex(const uint8_t* data, int32_t len) {
	...
	if (developerSysexCodeReceived && (data[1] != 0x7D)) {
		...
		connectedDevice->bufferMessage(packed, MIDI_SEND_PRIORITY_SYSEX);
		pos = 6;
	}
