...
void ConnectedUSBMIDIDevice::bufferMessage(uint32_t fullMessage, uint8_t priority) {
	// Defensive fallback: unknown priority values are treated as regular CC lane.
	if (priority >= MIDI_SEND_PRIORITY_COUNT) {
		priority = MIDI_SEND_PRIORITY_CC;
	}
...
int ConnectedUSBMIDIDevice::sendBufferSpace() {
	...
	uint32_t totalCapacity = (MIDI_SEND_BUFFER_LEN_RING - 1) * MIDI_SEND_PRIORITY_COUNT;
