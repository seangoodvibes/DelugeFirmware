...
public:
	...
	/// Shared CC gate helper: if head is CC and budget allows, attempt a scheduled pop.
	///
	...
	/// scheduler when the caller's per-transfer/per-UART budget still permits it.
	template <typename PopFn, typename... Args>
	static CCScheduledPopResult try_pop_scheduled_cc(bool head_is_cc, bool budget_ok, PopFn pop_scheduled_fn,
	                                                 Args&&... args) {
		if (!head_is_cc) {
		...
		if (!budget_ok) {
			...
			return CCScheduledPopResult::BudgetBlocked;
		}
		...
		// The head was CC and budget was available, but the transport could not pop one.
...
public:
	...
	struct USBSendContext {
		...
		int32_t& cc_budget_messages_remaining;
	};
...
private:
	...
	static constexpr uint16_t k_serial_queue_capacity = 512;
	...
	/// Pops the next scheduled complete CC message while respecting DIN budget and UART space.
	[[nodiscard]] bool pop_next_scheduled_cc_message(uint8_t* out_bytes, int32_t budget_bytes, int32_t uart_space,
