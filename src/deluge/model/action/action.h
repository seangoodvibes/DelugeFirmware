...
public:
	...
	Error recordNoteExistenceChange(InstrumentClip* clip, int32_t noteRowId, Note* note, ExistenceChangeType type,
	                                Note** recorded_note = nullptr, void** prepared_memory = nullptr);
	// Caller owns unused storage; recording consumes and clears it when used.
...
private:
	bool record_param_change(ModelStackWithAutoParam const* model_stack, bool steal_data, bool only_if_missing);
};
