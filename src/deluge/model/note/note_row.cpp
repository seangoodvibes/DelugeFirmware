...
Error NoteRow::deleteNoteByPos(ModelStackWithNoteRow* modelStack, int32_t pos, Action* action, void** prepared_memory) {
	if (!notes.getNumElements()) {
		return Error::NONE;
	}
	...
	Error error = deleteNoteByIndex(i, action, modelStack->noteRowId,
	                                ((InstrumentClip*)modelStack->getTimelineCounter()), prepared_memory);
	if (error != Error::NONE)
		return error;
