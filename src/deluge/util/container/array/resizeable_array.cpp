...
void ResizeableArray::delete_at_index_preserving_capacity(int32_t index, int32_t count) {
	if (index < 0 || count <= 0 || index > numElements || count > numElements - index)
		return;
	LOCK_ENTRY
	// Logical forward copying also works when the occupied region wraps in memory.
	for (int32_t destination = index; destination < numElements - count; ++destination) {
		memcpy(getElementAddress(destination), getElementAddress(destination + count), elementSize);
	}
	numElements -= count;
	LOCK_EXIT
}
