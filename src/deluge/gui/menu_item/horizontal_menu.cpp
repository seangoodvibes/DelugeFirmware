...
ActionResult HorizontalMenu::buttonAction(hid::Button b, bool on, bool inCardRoutine) {
	...
	static ui_session::State<double> navigation_button_times;
	auto& last_navigation_buttons_press_time = navigation_button_times.active();
	...
	for (size_t n = 0; n < visible_page_items.size(); n++) {
		...
		if (pressed_button_position >= current_column
			...
			if (permission == MenuPermission::MUST_SELECT_RANGE
				...
				multiRangeMenu.destination_for_session() = *current_item_iterator();
...
void HorizontalMenu::handleItemAction(MenuItem* menuItem) {
	...
	if (permission == MenuPermission::MUST_SELECT_RANGE) {
		sound_editor_for_session().currentMultiRange = nullptr;
		multiRangeMenu.destination_for_session() = menuItem;
