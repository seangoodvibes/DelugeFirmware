...
bool& moving_cursor_for_session() {
	static deluge::gui::ui_session::State<bool> moving_cursors;
	return moving_cursors.active();
}
...
void Decimal::horizontalEncoderAction(int32_t offset) {
	...
	if (display->haveOLED()) {
		auto& moving_cursor = moving_cursor_for_session();
		const bool previous_moving_cursor = moving_cursor;
		moving_cursor = true;
		renderUIsForOled();
		moving_cursor = previous_moving_cursor;
	}
	else {
		scrollToGoodPos();
...
void Decimal::drawPixelsForOled() {
	...
	hid::display::OLED::main_for_session().drawString(buffer, stringStartX, 20, digitWidth, kTextHugeSizeY, 0, 128,
	                                                  true);
	// draw cursor
	hid::display::OLED::setupBlink(ourDigitStartX + 1, digitWidth - 2, 41, 42, moving_cursor_for_session());
}
