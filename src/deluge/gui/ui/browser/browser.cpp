...
struct Browser::SessionState {
	int32_t max_file_items{};
	int32_t catalog_search_direction{};
	String currentDir{};
...
int32_t& Browser::max_file_items_for_session() {
	return session_state().max_file_items;
}
int32_t& Browser::catalog_search_direction_for_session() {
	return session_state().catalog_search_direction;
}
...
FileItem* Browser::getNewFileItem() {
	...
	if (file_items_for_session().getNumElements() >= max_file_items_for_session()) {
doCull:
		cullSomeFileItems();
		alreadyCulled = true;
	}
...
void Browser::cullSomeFileItems() {
	...
	int32_t numFileItemsDeletingNow =
	    file_items_for_session().getNumElements() - (max_file_items_for_session() >> 1); // May get modified below.
	if (numFileItemsDeletingNow <= 0) {
		return;
	}

	// If we already know what side we want to be deleting on...
	if (catalog_search_direction_for_session() == CATALOG_SEARCH_LEFT) {
deleteFromLeftSide:
	...
	else if (catalog_search_direction_for_session() == CATALOG_SEARCH_RIGHT) {
deleteFromRightSide:
...
Error Browser::readFileItemsForFolder(char const* filePrefixHere, bool allowFolders,
	...
	catalog_search_direction_for_session() = newCatalogSearchDirection;
	max_file_items_for_session() = newMaxNumFileItems;
