...
#include "gui/ui/load/load_note_row_sequence_data_ui.h"
...
#define NOTE_ROW_SEQUENCE_DATA_DEFAULT_FOLDER "SEQUENCES/"

LoadNoteRowSequenceDataUI loadNoteRowSequenceDataUI{};

bool LoadNoteRowSequenceDataUI::getGreyoutColsAndRows(uint32_t* cols, uint32_t* rows) {
	*cols = 0xFFFFFFFF;
	return true;
}

bool LoadNoteRowSequenceDataUI::opened() {
	...
	error = setupForLoadingNoteRowSequenceData(); // Sets currentDir.
	if (error != Error::NONE) {
		renderingNeededRegardlessOfUI(); // Because unlike many UIs we've already gone and drawn the QWERTY interface on
		                                 // the pads, in call to setupForLoadingNoteRowSequenceData().
		display->displayError(error);
		return false;
	}
...
Error LoadNoteRowSequenceDataUI::setupForLoadingNoteRowSequenceData() {
	// reset
	fileIconPt2 = nullptr;
	fileIconPt2Width = 0;
	...
	if (noteRow->sequenceDataFileName.isEmpty()) {
...
void LoadNoteRowSequenceDataUI::folderContentsReady(int32_t entryDirection) {
}

void LoadNoteRowSequenceDataUI::enterKeyPress() {
...
ActionResult LoadNoteRowSequenceDataUI::buttonAction(deluge::hid::Button b, bool on, bool inCardRoutine) {
...
ActionResult LoadNoteRowSequenceDataUI::padAction(int32_t x, int32_t y, int32_t on) {
	if (x < kDisplayWidth) {
...
Error LoadNoteRowSequenceDataUI::performLoad(bool doClone) {
