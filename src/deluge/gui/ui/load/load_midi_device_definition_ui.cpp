...
#define MIDI_DEVICES_DEFAULT_FOLDER "MIDI/DEVICES"
...
Error LoadMidiDeviceDefinitionUI::setupForLoadingMidiDeviceDefinition() {
	...
	if (midiInstrument->deviceDefinitionFileName.isEmpty()) {
		Error error = currentDir.set(MIDI_DEVICES_DEFAULT_FOLDER);
		if (error != Error::NONE) {
			return error;
		}
	}
	...
	Error error = arrivedInNewFolder(0, searchFilename.get(), MIDI_DEVICES_DEFAULT_FOLDER);
	if (error != Error::NONE) {
		return error;
	}
...
ActionResult LoadMidiDeviceDefinitionUI::buttonAction(deluge::hid::Button b, bool on, bool inCardRoutine) {
	...
	if (b == LOAD) {
		...
		if (on && b == BACK) {
			...
			if (!strcmp(currentDir.get(), MIDI_DEVICES_DEFAULT_FOLDER)) {
				close();
