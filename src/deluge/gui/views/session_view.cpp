...
void SessionView::reset_for_remote_startup() {
	instrument_clip_view_for_session().reset_selection_display_for_session_startup();
	automation_view_for_session().reset_navigation_for_session_startup();
	automation_view_for_session().reset_gestures_for_session_startup();
	sound_editor_for_session().reset_for_session_startup();
	view_for_session().reset_midi_learn_for_session_startup();
	view_for_session().reset_modulation_for_session_startup();
	...
	selectedClipPressYDisplay = 255;
	selectedClipPressXDisplay = 255;
	selectedClipTimePressed = 0;
}
			...
			if (clip->getCurrentlyRecordingLinearly()) {
				...
				if (newTickSquare >= 0
				    && (clip->armState == ArmState::OFF
				        || recording_scroll_for_session() != -1)) { // Only if it's auto extending, or it was before
	...
	if (!anyLinearRecordingOnThisScreen && currentUIMode != UI_MODE_HORIZONTAL_SCROLL) {
		...
		if (anyLinearRecordingOnNextScreen) {
			...
			if (!currentUIMode && getCurrentUI() == this) {
				...
				if (recording_scroll_for_session() == -1) {
					recording_scroll_for_session() = currentSong->x_scroll_for_session()[NAVIGATION_CLIP];
				}
		...
		// Or if not, cancel following scrolling along, and go back to where we started
			...
			if (recording_scroll_for_session() != -1) {
				int32_t newXScroll = recording_scroll_for_session();
				recording_scroll_for_session() = -1;
...
void SessionView::potentiallyRenderClipLaunchPlayhead(bool reallyNoTickSquare, int32_t sixteenthNotesRemaining) {
	std::array<uint8_t, kDisplayHeight> launch_tick_squares;
	launch_tick_squares.fill(255);
	int32_t newTickSquare;
	...
	if (renderPlayhead && (sixteenthNotesRemaining > 0 && sixteenthNotesRemaining <= kDisplayWidth)) {
		...
		launch_tick_squares[kDisplayHeight - 1] = newTickSquare;
	}
	// don't render playhead
	else {
		launch_tick_squares[kDisplayHeight - 1] = 255;
	}

	PadLEDs::setTickSquares(launch_tick_squares.data(), launchTickColours);
}
