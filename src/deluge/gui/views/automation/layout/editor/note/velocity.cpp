...
void AutomationLayoutEditorNoteVelocity::renderNoteColumn(ModelStackWithNoteRow* modelStackWithNoteRow,
                                                          InstrumentClip* clip,
...
void AutomationLayoutEditorNoteVelocity::renderNoteSquare(RGB image[][kDisplayWidth + kSideBarWidth],
                                                          uint8_t occupancyMask[][kDisplayWidth + kSideBarWidth],
...
void AutomationLayoutEditorNoteVelocity::renderNoteEditorDisplayOLED(deluge::hid::display::oled_canvas::Canvas& canvas,
...
void AutomationLayoutEditorNoteVelocity::renderNoteEditorDisplay7SEG(InstrumentClip* clip, OutputType outputType,
                                                                     int32_t knobPosLeft) {
	char modelStackMemory[MODEL_STACK_MAX_SIZE];
	ModelStackWithTimelineCounter* modelStack = currentSong->setupModelStackWithCurrentClip(modelStackMemory);
	bool isKit = outputType == OutputType::KIT;
...
void AutomationLayoutEditorNoteVelocity::velocityEditPadAction(ModelStackWithNoteRow* modelStackWithNoteRow,
	...
	// otherwise, it's a regular velocity pad action
		...
		// pressing pad corresponding to note's current averageVelocity, remove note
			...
			automationLayoutEditorNote.recordNoteEditPadAction(x, velocity);
			refreshVelocityEditor = true;
...
int32_t AutomationLayoutEditorNoteVelocity::getVelocityFromY(int32_t y) {
	int32_t velocity = std::clamp<int32_t>(nonPatchCablePadPressValues[y], 1, 127);
	return velocity;
}

// convert velocity of a square into y
int32_t AutomationLayoutEditorNoteVelocity::getYFromVelocity(int32_t velocity) {
...
// add note and set velocity
void AutomationLayoutEditorNoteVelocity::addNoteWithNewVelocity(int32_t x, int32_t velocity, int32_t newVelocity) {
	if (velocity) {
...
void AutomationLayoutEditorNoteVelocity::adjustNoteVelocity(ModelStackWithNoteRow* modelStackWithNoteRow,
...
void AutomationLayoutEditorNoteVelocity::setVelocity(ModelStackWithNoteRow* modelStackWithNoteRow, NoteRow* noteRow,
...
void AutomationLayoutEditorNoteVelocity::setVelocityRamp(ModelStackWithNoteRow* modelStackWithNoteRow, NoteRow* noteRow,
                                                         SquareInfo rowSquareInfo[kDisplayWidth],
