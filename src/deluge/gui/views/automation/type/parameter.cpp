...
void ParameterAutomation::initMIDICCShortcutsForAutomation() {
...
bool ParameterAutomation::opened() {
	initializeView();
...
void ParameterAutomation::initializeView() {
...
void ParameterAutomation::focusRegained() {
	if (onArrangerView) {
...
void ParameterAutomation::openedInBackground() {
	Clip* clip = getCurrentClip();
	...
	AudioEngine::logAction("ParameterAutomation::beginSession 2");
...
void ParameterAutomation::graphicsRoutine() {
	if (onArrangerView) {
...
AutomationSubType ParameterAutomation::getAutomationSubType() {
	if (onArrangerView) {
...
// rendering
bool ParameterAutomation::possiblyRefreshAutomationEditorGrid(Clip* clip, params::Kind paramKind, int32_t paramID) {
...
void ParameterAutomation::renderNoteEditor(ModelStackWithNoteRow* modelStackWithNoteRow, InstrumentClip* clip,
                                           RGB image[][kDisplayWidth + kSideBarWidth],
...
void ParameterAutomation::renderNoteColumn(ModelStackWithNoteRow* modelStackWithNoteRow, InstrumentClip* clip,
                                           RGB image[][kDisplayWidth + kSideBarWidth],
...
void ParameterAutomation::renderNoteSquare(RGB image[][kDisplayWidth + kSideBarWidth],
                                           uint8_t occupancyMask[][kDisplayWidth + kSideBarWidth], int32_t xDisplay,
...
bool ParameterAutomation::renderSidebar(uint32_t whichRows, RGB image[][kDisplayWidth + kSideBarWidth],
                                        uint8_t occupancyMask[][kDisplayWidth + kSideBarWidth]) {
	if (onArrangerView) {
...
void ParameterAutomation::renderAutomationOverviewDisplayOLED(deluge::hid::display::oled_canvas::Canvas& canvas,
...
void ParameterAutomation::renderNoteEditorDisplayOLED(deluge::hid::display::oled_canvas::Canvas& canvas,
...
void ParameterAutomation::renderAutomationOverviewDisplay7SEG(Output* output, OutputType outputType) {
	char const* overviewText;
	if (!onArrangerView && (outputType == OutputType::KIT && !getAffectEntire() && !((Kit*)output)->selectedDrum)) {
		overviewText = l10n::get(l10n::String::STRING_FOR_SELECT_A_ROW_OR_AFFECT_ENTIRE);
	}
...
void ParameterAutomation::renderNoteEditorDisplay7SEG(InstrumentClip* clip, OutputType outputType,
                                                      int32_t knobPosLeft) {
	char modelStackMemory[MODEL_STACK_MAX_SIZE];
	ModelStackWithTimelineCounter* modelStack = currentSong->setupModelStackWithCurrentClip(modelStackMemory);
	bool isKit = outputType == OutputType::KIT;
...
ActionResult ParameterAutomation::buttonAction(hid::Button b, bool on, bool inCardRoutine) {
	if (inCardRoutine) {
...
void ParameterAutomation::handleSessionButtonAction(Clip* clip, bool on) {
...
void ParameterAutomation::handleKeyboardButtonAction(bool on) {
...
void ParameterAutomation::handleClipButtonAction(bool on, bool isAudioClip) {
...
void ParameterAutomation::handleCrossScreenButtonAction(bool on) {
...
void ParameterAutomation::handleKitButtonAction(OutputType outputType, bool on) {
	if (on && (currentUIMode == UI_MODE_NONE || (currentUIMode == UI_MODE_NOTES_PRESSED && padSelectionOn))) {
...
void ParameterAutomation::handleSynthButtonAction(OutputType outputType, bool on) {
	if (on && (currentUIMode == UI_MODE_NONE || (currentUIMode == UI_MODE_NOTES_PRESSED && padSelectionOn))) {
		// if you're going to create a new instrument or change output type,
		// reset selection
		initParameterSelection();
...
void ParameterAutomation::handleMidiButtonAction(OutputType outputType, bool on) {
	if (on && (currentUIMode == UI_MODE_NONE || (currentUIMode == UI_MODE_NOTES_PRESSED && padSelectionOn))) {
...
void ParameterAutomation::handleCVButtonAction(OutputType outputType, bool on) {
	if (on && (currentUIMode == UI_MODE_NONE || (currentUIMode == UI_MODE_NOTES_PRESSED && padSelectionOn))) {
		// if you're going to change output type,
		// reset selection
		initParameterSelection();
...
bool ParameterAutomation::handleHorizontalEncoderButtonAction(bool on, bool isAudioClip) {
	if (onArrangerView) {
		return true;
	}
	else if (isAudioClip) {
...
bool ParameterAutomation::handleBackAndHorizontalEncoderButtonComboAction(Clip* clip, bool on) {
...
void ParameterAutomation::handleVerticalEncoderButtonAction(bool on) {
	if (on) {
		if (inNoteEditor()) {
...
void ParameterAutomation::handleSelectEncoderButtonAction(bool on) {
	if (on && (currentUIMode == UI_MODE_NONE || (currentUIMode == UI_MODE_NOTES_PRESSED && padSelectionOn))) {
		initParameterSelection();
		uiNeedsRendering(this);
...
bool ParameterAutomation::toggleVelocityPadSelectionMode(SquareInfo& squareInfo) {
...
void ParameterAutomation::noteEditPadAction(ModelStackWithNoteRow* modelStackWithNoteRow, NoteRow* noteRow,
...
void ParameterAutomation::velocityPadSelectionAction(ModelStackWithNoteRow* modelStackWithNoteRow, InstrumentClip* clip,
...
void ParameterAutomation::velocityEditPadAction(ModelStackWithNoteRow* modelStackWithNoteRow, NoteRow* noteRow,
                                                InstrumentClip* clip, int32_t x, int32_t y, int32_t velocity,
                                                int32_t effectiveLength, SquareInfo& squareInfo) {
	// save pad selected
	leftPadSelectedX = x;
...
int32_t ParameterAutomation::getVelocityFromY(int32_t y) {
	int32_t velocity = std::clamp<int32_t>(nonPatchCablePadPressValues[y], 1, 127);
	return velocity;
}

// convert velocity of a square into y
int32_t ParameterAutomation::getYFromVelocity(int32_t velocity) {
...
// add note and set velocity
void ParameterAutomation::addNoteWithNewVelocity(int32_t x, int32_t velocity, int32_t newVelocity) {
	if (velocity) {
...
void ParameterAutomation::adjustNoteVelocity(ModelStackWithNoteRow* modelStackWithNoteRow, NoteRow* noteRow, int32_t x,
...
void ParameterAutomation::setVelocity(ModelStackWithNoteRow* modelStackWithNoteRow, NoteRow* noteRow, int32_t x,
                                      int32_t newVelocity) {
...
void ParameterAutomation::setVelocityRamp(ModelStackWithNoteRow* modelStackWithNoteRow, NoteRow* noteRow,
...
void ParameterAutomation::recordNoteEditPadAction(int32_t x, int32_t velocity) {
...
ActionResult ParameterAutomation::handleMutePadAction(ModelStackWithTimelineCounter* modelStackWithTimelineCounter,
...
ActionResult ParameterAutomation::handleAuditionPadAction(InstrumentClip* instrumentClip, Output* output,
                                                          OutputType outputType, int32_t y, int32_t velocity) {
	if (onArrangerView) {
		if (onAutomationOverview()) {
...
void ParameterAutomation::auditionPadAction(int32_t velocity, int32_t yDisplay, bool shiftButtonDown) {
...
void ParameterAutomation::noteRowChanged(InstrumentClip* clip, NoteRow* noteRow) {
	instrumentClipView.noteRowChanged(clip, noteRow);
}

// called by playback_handler.cpp
void ParameterAutomation::notifyPlaybackBegun() {
...
uint32_t ParameterAutomation::getMaxLength() {
	if (onArrangerView) {
...
uint32_t ParameterAutomation::getMaxZoom() {
	if (onArrangerView) {
...
int32_t ParameterAutomation::getNavSysId() const {
	if (onArrangerView) {
...
bool ParameterAutomation::onAutomationOverview() {
	return (!inAutomationEditor() && !inNoteEditor());
}

bool ParameterAutomation::inAutomationEditor() {
	if (onArrangerView) {
...
void ParameterAutomation::setAutomationParamType() {
	automationParamType = AutomationParamType::PER_SOUND;
	if (!inAutomationEditor()) {
		Clip* clip = getCurrentClip();
...
bool ParameterAutomation::inNoteEditor() {
