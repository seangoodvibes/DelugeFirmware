...
void View::reset_midi_learn_for_session_startup() {
	uiTimerManager.unsetTimer(TimerName::MIDI_LEARN_FLASH);
	thingPressedForMidiLearn = MidiLearn::NONE;
	deleteMidiCommandOnRelease = false;
	midiLearnFlashOn = false;
	learnedThing = nullptr;
	instrumentPressedForMIDILearn = nullptr;
	drumPressedForMIDILearn = nullptr;
	kitPressedForMIDILearn = nullptr;
	// Preserve shouldSaveSettingsAfterMidiLearn: completed edits may still need saving.
}

void View::reset_modulation_for_session_startup() {
	// Root opening may render before it binds the new song modulation target.
	memset(&activeModControllableModelStack, 0, sizeof(activeModControllableModelStack));
	memset(dummy, 0, sizeof(dummy));
	modLength = 0;
	modPos = 0xFFFFFFFF;
	modNoteRowId = 0;
	pendingParamAutomationUpdatesModLevels = false;
