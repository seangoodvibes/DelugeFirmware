...
class SessionRoutingContracts(unittest.TestCase):
    def test_shortcut_colours_read_the_selected_drum_for_the_active_session(self):
        ...
        start = source.index("ModControllableAudio* InstrumentClipView::getModControllableAudioOrNone()")
        end = source.index("void InstrumentClipView::deleteDrum", start)
        method = code_only(source[start:end])
        self.assertIn("static_cast<Kit*>(output)->selected_drum_for_session()", method)
        self.assertNotRegex(method, r"\bselectedDrum\b")
