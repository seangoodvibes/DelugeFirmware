...
TEST(IndicatorRuntime, reconnect_clears_remote_blink_records_and_timers_without_output_or_local_changes) {
	for (auto owner : {us::Id::Local, us::Id::Remote}) {
		us::Scope scope(owner);
		blinkLed(LED::PLAY, 255, 0, true);
		blinkLed(LED::SAVE, 255, 1, true);
		blinkKnobIndicator(1, true);
		setKnobIndicatorLevel(0, 100, false);
	}
	const auto local_frame = frame_for_session();
	const auto led_writes = fixture::leds.size();
	const auto knob_writes = fixture::knobs.size();
	{
		us::Scope remote(us::Id::Remote);
		const auto old_frame = frame_for_session();
		reset_blinking_for_session();
		reset_blinking_for_session();
		LONGS_EQUAL(255, getLedBlinkerIndex(LED::PLAY));
		LONGS_EQUAL(255, getLedBlinkerIndex(LED::SAVE));
		CHECK_FALSE(isKnobIndicatorBlinking(1));
		for (auto timer : {TimerName::LED_BLINK, TimerName::LED_BLINK_TYPE_1, TimerName::LEVEL_INDICATOR_BLINK,
		                   TimerName::METER_INDICATOR_BLINK})
			CHECK_FALSE(uiTimerManager.isTimerSet(timer));
		CHECK(old_frame.leds == frame_for_session().leds);
		CHECK(old_frame.knobs == frame_for_session().knobs);
		UNSIGNED_LONGS_EQUAL(old_frame.revision, frame_for_session().revision);
		blinkLed(LED::RECORD, 255, 0, false);
		ledBlinkTimeout(0);
		LONGS_EQUAL(255, getLedBlinkerIndex(LED::PLAY));
		CHECK_TRUE(frame_for_session().leds[static_cast<size_t>(LED::PLAY)]);
	}
	...
	CHECK(local_frame.leds == frame_for_session().leds);
	CHECK(local_frame.knobs == frame_for_session().knobs);
	CHECK(getLedBlinkerIndex(LED::PLAY) != 255);
	CHECK(getLedBlinkerIndex(LED::SAVE) != 255);
	CHECK_TRUE(isKnobIndicatorBlinking(1));
	for (auto timer : {TimerName::LED_BLINK, TimerName::LED_BLINK_TYPE_1, TimerName::LEVEL_INDICATOR_BLINK,
	                   TimerName::METER_INDICATOR_BLINK})
		CHECK_TRUE(uiTimerManager.isTimerSet(timer));
}
