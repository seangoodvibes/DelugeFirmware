...
TEST_GROUP(MirrorRuntime) {
	...
	void setup() override {
		...
		on_root_open = on_root_lookup = on_root_install = {};
		on_cancel_popup = on_remove_working_animation = on_reset_layers = {};
		physical_display.retained_layers = {};
		physical_display.scrolling = physical_display.blinking = {};
		root_open_success = true;
		remote_root.pulsing = false;
		remote_root.pulse_resets = 0;
		session::navigation.for_owner(session::Id::Remote) = {};
		on_remote_render = on_ui_timers = {};
		defer_ui_render = false;
		defer_oled_publication = false;
		m::active_session_mode = p::session_mode::visible_host;
		deluge::hid::display::OLED::remote_frame_available = false;
		deluge::hid::display::OLED::remote_revision = 0;
...
TEST(MirrorRuntime, oled_snapshot_waits_for_fresh_publication_even_when_dirty_flags_clear) {
	for (bool stale_frame : {false, true}) {
		incoming(p::Op::Request, 0, {1});
		m::active_session_mode = p::session_mode::independent;
		session::navigation.for_owner(session::Id::Remote).depth = 1;
		deluge::hid::display::OLED::remote_frame_available = stale_frame;
		defer_oled_publication = true;
		cable.sent.clear();
		m::routine();
		CHECK_TRUE(m::snapshot_pending);
		CHECK_FALSE(m::failed);
		CHECK_TRUE(sent(p::Op::Accept).empty());
		CHECK_FALSE(session::navigation.for_owner(session::Id::Remote).oled_dirty);
		defer_oled_publication = false;
		m::routine();
		CHECK_FALSE(m::snapshot_pending);
		LONGS_EQUAL(1, sent(p::Op::Accept).size());
...
TEST(MirrorRuntime, numeric_snapshot_does_not_wait_for_oled_publication) {
	physical_display.oled = false;
	incoming(p::Op::Request, 0, {0});
	m::active_session_mode = p::session_mode::independent;
	session::navigation.for_owner(session::Id::Remote).depth = 1;
...
TEST(MirrorRuntime, oled_snapshot_accepts_wrapped_publication_revision) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	session::navigation.for_owner(session::Id::Remote).depth = 1;
	deluge::hid::display::OLED::remote_revision = UINT32_MAX;
	m::routine();
	CHECK_FALSE(m::snapshot_pending);
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
}

TEST(MirrorRuntime, oled_snapshot_rejects_changed_revision_without_copyable_frame) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	session::navigation.for_owner(session::Id::Remote).depth = 1;
	defer_oled_publication = true;
	on_remote_render = [] {
		++deluge::hid::display::OLED::remote_revision;
		deluge::hid::display::OLED::remote_frame_available = false;
	};
	m::routine();
	CHECK_TRUE(m::snapshot_pending);
...
TEST(MirrorRuntime, blocked_acceptance_does_not_accumulate_preparation_panel_output) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	session::navigation.for_owner(session::Id::Remote).depth = 1;
	cable.space = 0;
	...
	for (int attempt = 0; attempt < 16; ++attempt) {
		...
		CHECK_FALSE(m::failed);
		CHECK_TRUE(m::snapshot_pending);
		LONGS_EQUAL(m::panel_read, m::panel_write);
		CHECK_FALSE(m::preparing_remote_snapshot);
	}
	LONGS_EQUAL(16, renders);
	LONGS_EQUAL(5, m::remote_panel.lengths[224]);
	LONGS_EQUAL(7, m::remote_panel.commands[224][1]);
	on_remote_render = {};
	cable.space = 65536;
	m::routine();
	CHECK_FALSE(m::failed);
	...
	m::transport_routine();
	CHECK_FALSE(sent(p::Op::Panel).empty());
}

TEST(MirrorRuntime, independent_disconnect_cancels_all_remote_timers_after_release_without_touching_local) {
	host();
	m::active_session_mode = p::session_mode::independent;
	for (int timer_index = 0; timer_index < static_cast<int>(TimerName::NUM_TIMERS); ++timer_index)
		uiTimerManager.setTimer(static_cast<TimerName>(timer_index), 100 + timer_index);
	const auto local_before = uiTimerManager.state.bank(session::Id::Local);
	m::remote_held[2] = true;
	int releases = 0;
	fixture::on_input = [&] {
		++releases;
		CHECK_TRUE(session::current() == session::Id::Remote);
		for (int timer_index = 0; timer_index < static_cast<int>(TimerName::NUM_TIMERS); ++timer_index) {
			auto timer = static_cast<TimerName>(timer_index);
			if (UITimerState::owner(timer) == session::Id::Remote)
				uiTimerManager.setTimer(timer, 1);
		}
	};
	m::stop("test");
	fixture::on_input = {};
	LONGS_EQUAL(1, releases);
	const auto& local_after = uiTimerManager.state.bank(session::Id::Local);
	const auto& remote_after = uiTimerManager.state.bank(session::Id::Remote);
	for (size_t index = 0; index < local_before.timers.size(); ++index) {
		CHECK_TRUE(local_before.timers[index].active == local_after.timers[index].active);
		UNSIGNED_LONGS_EQUAL(local_before.timers[index].triggerTime, local_after.timers[index].triggerTime);
		CHECK_FALSE(remote_after.timers[index].active);
	}
	UNSIGNED_LONGS_EQUAL(local_before.next_event, local_after.next_event);
	CHECK_TRUE(session::current() == session::Id::Local);
}

TEST(MirrorRuntime, disconnect_detaches_remote_navigation_after_release_and_preserves_local) {
	host();
	m::active_session_mode = p::session_mode::independent;
	auto& local_navigation = session::navigation.for_owner(session::Id::Local);
	auto& remote_navigation = session::navigation.for_owner(session::Id::Remote);
	local_navigation.depth = 1;
	local_navigation.hierarchy[0] = &ui;
	local_navigation.mode = 7;
	remote_navigation.depth = 1;
	remote_navigation.hierarchy[0] = &ui;
	remote_navigation.rendering = true;
	const auto revision = remote_navigation.structural_refresh.revision();
	m::remote_held[2] = true;
	fixture::on_input = [&] {
		LONGS_EQUAL(1, remote_navigation.depth);
		remote_navigation.depth = 2;
		remote_navigation.hierarchy[1] = &ui;
		remote_navigation.last_before_nullifying = &ui;
		remote_navigation.mode = 3;
	};
	m::stop("test");
	fixture::on_input = {};
	LONGS_EQUAL(0, remote_navigation.depth);
	for (auto* entry : remote_navigation.hierarchy)
		POINTERS_EQUAL(nullptr, entry);
	POINTERS_EQUAL(nullptr, remote_navigation.last_before_nullifying);
	LONGS_EQUAL(0, remote_navigation.mode);
	CHECK_TRUE(remote_navigation.rendering);
	CHECK_TRUE(remote_navigation.structural_refresh.revision() != revision);
	LONGS_EQUAL(1, local_navigation.depth);
	POINTERS_EQUAL(&ui, local_navigation.hierarchy[0]);
	LONGS_EQUAL(7, local_navigation.mode);
	CHECK_FALSE(m::remote_ui_ready());
	// Only fresh navigation initialization may make another independent session ready.
	remote_navigation.rendering = false;
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	cable.sent.clear();
	m::routine();
...
TEST(MirrorRuntime, independent_startup_opens_remote_song_root_before_acceptance_once) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	...
	on_root_open = [&] {
		...
		CHECK_TRUE(session::current() == session::Id::Remote);
		CHECK_TRUE(sent(p::Op::Accept).empty());
		POINTERS_EQUAL(&remote_root, session::navigation.active().hierarchy[0]);
	};
	m::routine();
	LONGS_EQUAL(1, openings);
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
	CHECK_FALSE(m::failed);
	CHECK_FALSE(m::remote_initialization_pending);
	POINTERS_EQUAL(&ui, local_navigation.hierarchy[0]);
	LONGS_EQUAL(1, local_navigation.depth);
	m::routine();
	LONGS_EQUAL(1, openings);
...
TEST(MirrorRuntime, failed_remote_root_open_never_accepts_and_teardown_detaches_it) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	root_open_success = false;
	m::routine();
	CHECK_TRUE(m::failed);
	CHECK_TRUE(sent(p::Op::Accept).empty());
	m::routine();
...
TEST(MirrorRuntime, remote_root_open_song_change_aborts_before_snapshot) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	Song replacement;
	on_root_open = [&] { currentSong = &replacement; };
	m::routine();
	CHECK_TRUE(m::failed);
	CHECK_TRUE(sent(p::Op::Accept).empty());
	CHECK_TRUE(fixture::main_pad_owners.empty());
...
TEST(MirrorRuntime, remote_root_construction_song_change_never_installs_or_opens_root) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	Song replacement;
	...
	on_root_lookup = [&] { currentSong = &replacement; };
	m::routine();
	CHECK_TRUE(m::failed);
	LONGS_EQUAL(0, openings);
	auto& navigation = session::navigation.for_owner(session::Id::Remote);
	LONGS_EQUAL(0, navigation.depth);
	POINTERS_EQUAL(nullptr, navigation.hierarchy[0]);
	CHECK_TRUE(sent(p::Op::Accept).empty());
	CHECK_FALSE(m::preparing_remote_snapshot);
	CHECK_TRUE(session::current() == session::Id::Local);
	currentSong = &song;
}
TEST(MirrorRuntime, remote_root_construction_stop_never_installs_root) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	on_root_lookup = [&] { incoming(p::Op::Stop, 1); };
	m::routine();
	CHECK_TRUE(m::failed);
	LONGS_EQUAL(0, session::navigation.for_owner(session::Id::Remote).depth);
	CHECK_TRUE(sent(p::Op::Accept).empty());
...
TEST(MirrorRuntime, remote_root_construction_cannot_overwrite_callback_navigation) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	on_root_lookup = [] {
		auto& navigation = session::navigation.active();
		navigation.depth = 1;
		navigation.hierarchy[0] = &ui;
	};
	m::routine();
	CHECK_TRUE(m::failed);
	POINTERS_EQUAL(&ui, session::navigation.for_owner(session::Id::Remote).hierarchy[0]);
	CHECK_TRUE(sent(p::Op::Accept).empty());
}

TEST(MirrorRuntime, replaced_root_during_installation_is_not_opened_or_accepted) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	int openings = 0, cancellations = 0;
	on_cancel_popup = [&] { ++cancellations; };
	on_root_install = [] { session::navigation.active().hierarchy[0] = &ui; };
	on_root_open = [&] { ++openings; };
	m::routine();
	CHECK_TRUE(m::failed);
	LONGS_EQUAL(0, openings);
	LONGS_EQUAL(0, cancellations);
	CHECK_TRUE(sent(p::Op::Accept).empty());
	POINTERS_EQUAL(&ui, session::navigation.for_owner(session::Id::Remote).hierarchy[0]);
}
TEST(MirrorRuntime, replaced_root_during_opening_is_not_accepted) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	on_root_open = [] { session::navigation.active().hierarchy[0] = &ui; };
	m::routine();
	CHECK_TRUE(m::failed);
	CHECK_TRUE(sent(p::Op::Accept).empty());
	CHECK_TRUE(fixture::main_pad_owners.empty());
...
TEST(MirrorRuntime, opening_overlay_above_expected_remote_root_is_accepted) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	on_root_open = [] {
		auto& navigation = session::navigation.active();
		navigation.hierarchy[1] = &ui;
		navigation.depth = 2;
	};
	m::routine();
	CHECK_FALSE(m::failed);
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
	POINTERS_EQUAL(&remote_root, session::navigation.for_owner(session::Id::Remote).hierarchy[0]);
}

TEST(MirrorRuntime, reconnect_resets_remote_song_pulse_before_each_open) {
	int openings = 0;
	on_root_open = [&] {
		++openings;
		CHECK_TRUE(session::current() == session::Id::Remote);
		CHECK_FALSE(remote_root.pulsing);
		LONGS_EQUAL(openings, remote_root.pulse_resets);
		remote_root.pulsing = true;
	};
	for (int connection = 0; connection < 2; ++connection) {
		...
		CHECK_FALSE(m::failed);
		CHECK_FALSE(m::snapshot_pending);
		CHECK_TRUE(remote_root.pulsing);
		m::stop("test");
	}
	LONGS_EQUAL(2, openings);
	CHECK_TRUE(session::current() == session::Id::Local);
}

TEST(MirrorRuntime, remote_startup_clears_sticky_shift_before_open_without_changing_local) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	Buttons::sticky_shift.for_owner(session::Id::Local) = true;
	Buttons::sticky_shift.for_owner(session::Id::Remote) = true;
	on_root_open = [] { CHECK_FALSE(Buttons::sticky_shift.active()); };
	m::routine();
	CHECK_FALSE(m::failed);
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
	CHECK_TRUE(Buttons::sticky_shift.for_owner(session::Id::Local));
	CHECK_FALSE(Buttons::sticky_shift.for_owner(session::Id::Remote));
	Buttons::sticky_shift.for_owner(session::Id::Local) = false;
}

TEST(MirrorRuntime, remote_startup_cancels_retained_popup_before_open_for_both_display_types) {
	for (bool oled : {false, true}) {
		physical_display.oled = oled;
		incoming(p::Op::Request, 0, {static_cast<uint8_t>(oled)});
		m::active_session_mode = p::session_mode::independent;
		m::remote_initialization_pending = true;
		physical_display.popup_active.for_owner(session::Id::Local) = true;
		physical_display.popup_active.for_owner(session::Id::Remote) = true;
		int cancellations = 0;
		on_cancel_popup = [&] {
			++cancellations;
			CHECK_TRUE(session::current() == session::Id::Remote);
		};
		on_root_open = [] { CHECK_FALSE(physical_display.popup_active.active()); };
		m::routine();
		CHECK_FALSE(m::failed);
		LONGS_EQUAL(1, cancellations);
		CHECK_TRUE(physical_display.popup_active.for_owner(session::Id::Local));
		m::stop("test");
	}
}
TEST(MirrorRuntime, popup_cancel_callback_song_change_prevents_remote_open_and_accept) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	Song replacement;
	int openings = 0;
	on_cancel_popup = [&] { currentSong = &replacement; };
	on_root_open = [&] { ++openings; };
	m::routine();
	CHECK_TRUE(m::failed);
	LONGS_EQUAL(0, openings);
...
TEST(MirrorRuntime, invalidated_root_installation_skips_popup_cleanup_and_open) {
	for (bool replace_song : {false, true}) {
		...
		Song replacement;
		int callbacks = 0;
		on_cancel_popup = on_root_open = [&] { ++callbacks; };
		physical_display.popup_active.for_owner(session::Id::Remote) = true;
		on_root_install = [&] {
			if (replace_song)
				currentSong = &replacement;
		...
		CHECK_TRUE(physical_display.popup_active.for_owner(session::Id::Remote));
		CHECK_TRUE(sent(p::Op::Accept).empty());
		CHECK_FALSE(m::preparing_remote_snapshot);
		currentSong = &song;
		on_root_install = {};
...
TEST(MirrorRuntime, remote_startup_removes_working_animation_before_popup_and_open) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	physical_display.working_animation.for_owner(session::Id::Local) = true;
	physical_display.working_animation.for_owner(session::Id::Remote) = true;
	on_cancel_popup = on_root_open = [] { CHECK_FALSE(physical_display.working_animation.active()); };
	m::routine();
	CHECK_FALSE(m::failed);
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
	CHECK_TRUE(physical_display.working_animation.for_owner(session::Id::Local));
}
TEST(MirrorRuntime, working_animation_cleanup_invalidation_skips_popup_and_open) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	int callbacks = 0;
	on_remove_working_animation = [&] { incoming(p::Op::Stop, 1); };
	on_cancel_popup = on_root_open = [&] { ++callbacks; };
	m::routine();
	CHECK_TRUE(m::failed);
	LONGS_EQUAL(0, callbacks);
...
TEST(MirrorRuntime, remote_startup_clears_oled_console_before_open_and_preserves_local) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	auto& consoles = deluge::hid::display::OLED::console_items;
	consoles.for_owner(session::Id::Local) = 2;
	consoles.for_owner(session::Id::Remote) = 3;
	on_root_open = [&] { LONGS_EQUAL(0, consoles.active()); };
	m::routine();
	CHECK_FALSE(m::failed);
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
	LONGS_EQUAL(2, consoles.for_owner(session::Id::Local));
	consoles.for_owner(session::Id::Local) = 0;
}

TEST(MirrorRuntime, remote_startup_discards_layers_before_resuming_display_or_opening_root) {
	physical_display.oled = false;
	incoming(p::Op::Request, 0, {0});
	CHECK(m::state == m::State::Host);
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	physical_display.retained_layers.for_owner(session::Id::Local) = 2;
	physical_display.retained_layers.for_owner(session::Id::Remote) = 3;
	int callbacks = 0;
	on_remove_working_animation = on_cancel_popup = on_root_open = [&] {
		LONGS_EQUAL(0, physical_display.retained_layers.active());
		++callbacks;
	};
	m::routine();
	CHECK_FALSE(m::failed);
	LONGS_EQUAL(3, callbacks);
	LONGS_EQUAL(2, physical_display.retained_layers.for_owner(session::Id::Local));
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
}
TEST(MirrorRuntime, layer_reset_invalidation_skips_later_display_callbacks_and_root_open) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	int callbacks = 0;
	on_reset_layers = [&] { incoming(p::Op::Stop, 1); };
	on_remove_working_animation = on_cancel_popup = on_root_open = [&] { ++callbacks; };
	m::routine();
	CHECK_TRUE(m::failed);
	LONGS_EQUAL(0, callbacks);
	CHECK_TRUE(sent(p::Op::Accept).empty());
...
TEST(MirrorRuntime, oled_startup_stops_old_animations_before_root_open_and_snapshot_timers) {
	incoming(p::Op::Request, 0, {1});
	CHECK(m::state == m::State::Host);
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	for (auto owner : {session::Id::Local, session::Id::Remote}) {
		physical_display.scrolling.for_owner(owner) = true;
		physical_display.blinking.for_owner(owner) = true;
	}
	int openings = 0, timer_calls = 0;
	on_root_open = [&] {
		CHECK_FALSE(physical_display.scrolling.active());
		CHECK_FALSE(physical_display.blinking.active());
		++openings;
	};
	on_ui_timers = [&] {
		CHECK_FALSE(physical_display.scrolling.active());
	...
	CHECK(timer_calls > 0);
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
	CHECK_TRUE(physical_display.scrolling.for_owner(session::Id::Local));
	CHECK_TRUE(physical_display.blinking.for_owner(session::Id::Local));
}

TEST(MirrorRuntime, startup_clears_remote_indicator_blink_records_before_root_open) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	for (auto owner : {session::Id::Local, session::Id::Remote}) {
		session::Scope scope(owner);
		indicator_leds::blinkLed(indicator_leds::LED::SAVE, 255, 0, true);
		indicator_leds::blinkKnobIndicator(1, true);
	}
	int openings = 0;
	on_root_open = [&] {
		LONGS_EQUAL(255, indicator_leds::getLedBlinkerIndex(indicator_leds::LED::SAVE));
		CHECK_FALSE(indicator_leds::isKnobIndicatorBlinking(1));
		++openings;
	};
	m::routine();
	CHECK_FALSE(m::failed);
	LONGS_EQUAL(1, openings);
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
	CHECK(indicator_leds::getLedBlinkerIndex(indicator_leds::LED::SAVE) != 255);
	CHECK_TRUE(indicator_leds::isKnobIndicatorBlinking(1));
	indicator_leds::reset_blinking_for_session();
}

TEST(MirrorRuntime, remote_startup_waits_for_pad_render_lock_before_transition_reset) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	PadLEDs::rendering_locks.for_owner(session::Id::Remote) = true;
	m::routine();
	CHECK_TRUE(m::remote_initialization_pending);
	LONGS_EQUAL(0, PadLEDs::transition_resets.for_owner(session::Id::Remote));
	CHECK_TRUE(sent(p::Op::Accept).empty());
	PadLEDs::rendering_locks.for_owner(session::Id::Remote) = false;
	PadLEDs::rendering_locks.for_owner(session::Id::Local) = true;
	on_root_open = [] { LONGS_EQUAL(1, PadLEDs::transition_resets.active()); };
	m::routine();
	CHECK_FALSE(m::failed);
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
	LONGS_EQUAL(0, PadLEDs::transition_resets.for_owner(session::Id::Local));
	CHECK_TRUE(PadLEDs::rendering_locks.for_owner(session::Id::Local));
}

TEST(MirrorRuntime, startup_clears_retained_remote_button_bank_before_open_without_input_dispatch) {
	incoming(p::Op::Request, 0, {1});
	m::active_session_mode = p::session_mode::independent;
	m::remote_initialization_pending = true;
	Buttons::retained_button_hold.for_owner(session::Id::Local) = true;
	Buttons::retained_button_hold.for_owner(session::Id::Remote) = true;
	const auto inputs = fixture::events.size();
	int openings = 0;
	on_root_open = [&] {
		CHECK_FALSE(Buttons::retained_button_hold.active());
		++openings;
	};
	m::routine();
	CHECK_FALSE(m::failed);
	LONGS_EQUAL(1, openings);
	LONGS_EQUAL(1, sent(p::Op::Accept).size());
	LONGS_EQUAL(inputs, fixture::events.size());
	CHECK_TRUE(Buttons::retained_button_hold.for_owner(session::Id::Local));
	Buttons::retained_button_hold = {};
}
