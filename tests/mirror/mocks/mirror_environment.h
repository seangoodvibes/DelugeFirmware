...
inline std::function<void()> on_cancel_popup, on_remove_working_animation, on_reset_layers;
struct MockDisplay {
	deluge::gui::ui_session::State<int> retained_layers;
	deluge::gui::ui_session::State<bool> scrolling, blinking;
	void reset_layers_for_session() {
		retained_layers.active() = 0;
		if (oled) {
			scrolling.active() = blinking.active() = false;
		}
		if (on_reset_layers)
			on_reset_layers();
	}
	deluge::gui::ui_session::State<bool> working_animation;
	void removeWorkingAnimation() {
		working_animation.active() = false;
		if (on_remove_working_animation)
			on_remove_working_animation();
	}
	deluge::gui::ui_session::State<bool> popup_active;
	void cancelPopup() {
...
inline std::function<void()> on_root_open, on_root_lookup, on_root_install;
inline bool root_open_success = true;
struct UI {
	bool pulsing = false;
	int pulse_resets = 0;
	...
	bool opened() {
		if (on_root_open)
			on_root_open();
		return root_open_success;
	}
	void focusRegained() { ++fixture::focused; }
};
inline UI ui;
inline UI remote_root;
inline UI& session_view_for_session() {
	if (on_root_lookup)
		on_root_lookup();
	return remote_root;
}
inline void setRootUILowLevel(UI* root) {
...
inline bool defer_oled_publication = false;
inline void publish_mock_remote_frame();
inline void doAnyPendingUIRendering() {
	...
	if (!defer_ui_render) {
		...
		if (!defer_oled_publication)
			publish_mock_remote_frame();
	}
}
...
inline void reset_for_session_startup() {
	sticky_shift.active() = false;
	retained_button_hold.active() = false;
}
inline void clearShiftSticky() {
	sticky_shift.active() = false;
}
...
struct OLED {
	inline static deluge::gui::ui_session::State<int> console_items;
	static void clear_console_for_session() { console_items.active() = 0; }
	...
	inline static uint32_t remote_revision = 0;
	static uint32_t remote_frame_revision() { return remote_revision; }
...
namespace PadLEDs {
inline deluge::gui::ui_session::State<bool> rendering_locks;
inline deluge::gui::ui_session::State<int> transition_resets;
inline bool& rendering_lock_for_session() {
	return rendering_locks.active();
}
inline void reset_transitions_for_session() {
	++transition_resets.active();
}
