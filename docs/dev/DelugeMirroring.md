...
### Require a fresh OLED publication for initial snapshots

OLED snapshot preparation now requires the render pass to advance the Remote frame
revision and provide a copyable completed frame. Cleared dirty flags or an old
published frame alone no longer permit acceptance. Numeric displays do not depend
on OLED publication. Runtime tests cover missing/stale publication, retry, numeric
exemption, and revision wraparound. The mock renderer explicitly models publication
and can defer it independently of dirty-flag consumption. Remote root creation and
remaining readiness validation are unfinished; independent negotiation stays disabled.

### Avoid accumulating panel output during snapshot preparation retries

A scoped preparation guard now suppresses streamed panel commands while initial
Remote timers/rendering run, while preserving their cache updates. Initial snapshot
emission and later live output retain their normal queue path. This prevents
repeated renders under USB backpressure from overflowing an unsendable panel queue.
A runtime regression exercises sixteen blocked attempts with full pad output,
checks cache preservation and guard restoration, then verifies acceptance and live
output after recovery. Remote root creation remains unfinished and independent
negotiation stays disabled.

### Cancel Remote UI timers after disconnect callbacks

Independent teardown now cancels every Remote-owned UI timer after input release
callbacks, replacing the earlier graphics-only cancellation. This prevents stale
menu, display, or animation callbacks from running during reconnect preparation.
Hardware timers remain explicitly Local-owned. Runtime coverage rearms all Remote
timers during release and verifies their cancellation alongside unchanged Local
timer activity, deadlines, and next-event scheduling. Remote root reinitialization
must rearm the timers needed by its new UI; root creation remains unfinished and
independent negotiation stays disabled.

### Detach Remote navigation on disconnect

After releasing injected input, independent teardown now detaches all Remote
navigation pointers, clears depth/mode/pending pad rows, and requests a structural
refresh. The active-render guard is preserved for a suspended renderer to release.
Local navigation is unchanged. A runtime regression covers release-callback stack
changes, pointer removal, structural invalidation, Local isolation, and reconnect
waiting for fresh initialization. This does not destroy or reset owning UI objects;
Remote root creation and per-view initialization remain unfinished. Independent
negotiation stays disabled.

### Initialize an independent Remote song root

Accepted independent requests now schedule opening the separately constructed
Remote SessionView before snapshot preparation. Startup installs and opens the
root under Remote ownership, preserves Local navigation, checks song/session/UI
validity around opening, and fails closed on an unsuccessful open. Opening output
updates caches without accumulating pre-acceptance panel commands. Runtime tests
exercise one-time opening, Local isolation, failed opening, and song replacement
using a stub root; full SessionView opening still requires integration/hardware
validation. Negotiation remains disabled. This provides a song-overview starting
root, not a clone of the host's current screen. Exact-screen initialization,
per-view reconnect state, and remaining readiness gates are unfinished.

### Validate lazy Remote root construction before installation

Startup now rechecks song identity, host-session liveness, UI ownership, and empty
navigation immediately after obtaining the lazily constructed Remote SessionView.
Invalid construction callbacks can no longer publish a root before the existing
post-install validation runs. Runtime tests simulate song replacement, Stop, and
callback-installed navigation, verifying no acceptance or overwritten navigation.
These remain callback harness tests rather than full SessionView construction
integration. Exact-screen startup and per-view reconnect initialization remain
unfinished; independent negotiation stays disabled.

### Validate Remote startup root identity

Startup now checks that the installed root remains the expected Remote SessionView
both before and after opened(). A callback replacing/removing the root fails startup
before acceptance; an overlay above the same root is allowed. Runtime tests cover
replacement during installation and opening, no opening of a superseded root, and
valid overlay creation. Exact-screen startup, per-view reconnect initialization,
and full UI/hardware validation remain unfinished. Independent negotiation stays
disabled.

### Reset retained SessionView pulse state before reopening

Remote root startup now invokes SessionView's existing gridStopSelectedClipPulsing
before opening. That method clears the retained selected-clip pointer and pulse
flag as well as the timer, so focusRegained can start pulsing after reconnect.
The runtime startup harness verifies reset-before-open on two successive sessions;
it stubs the view method and does not validate full SessionView rendering. Other
per-view reconnect state, exact-screen startup, and hardware integration remain
unfinished. Independent negotiation stays disabled.

### Reset SessionView transient gestures on Remote startup

The startup reset now clears grid presses, pending pad/section release actions,
shift-selection and session-button flags, encoder/record-arming holds, clip-creation
state, and selected-pad coordinates/time, alongside pulse cleanup. Layout and
scroll preferences are retained. A test compiles the production reset body with
stub pulse/grid-reset collaborators; the existing runtime reconnect test verifies
it is called before reopening. Other views, exact-screen initialization, and
hardware integration remain unfinished. Independent negotiation stays disabled.

### Clear retained Remote sticky Shift before opening

Remote startup now calls the existing clearShiftSticky under Remote ownership
before resetting/opening SessionView. This clears latched/pressed Shift and marks
its state changed without altering the Local button bank. A production-function
test verifies the actual clear method against real Buttons::State storage; a
runtime test verifies reset-before-open and Local isolation. Other per-view
reconnect state, exact-screen startup, and hardware validation remain unfinished.
Independent negotiation stays disabled.

### Cancel retained Remote popups before opening

Startup now cancels the Remote display popup after root installation and before
opening. Existing post-install validation also covers callbacks from numeric popup
cancellation, which can resume the underlying layer. Runtime tests exercise both
display types, Local popup isolation, reset-before-open, and a cancellation callback
replacing the song. Tests stub display cancellation; full layer lifecycle and other
retained overlay state still need validation. Exact-screen startup and remaining
reconnect work are unfinished; independent negotiation stays disabled.

### Validate root installation before popup cleanup

Remote startup now checks the installed root before cancelling a retained popup,
then rechecks after cancellation and opening. A failed installation can no longer
run additional popup/layer callbacks. The shared validity check also requires Host
state. Runtime tests cover replacement of the root or song and Stop during
installation, verifying no popup cleanup, opening, or acceptance afterward.
Remaining overlay state, exact-screen startup, and hardware integration are still
unfinished; independent negotiation stays disabled.

### Remove retained working animations before Remote opening

Startup now removes the Remote working animation before popup cancellation, with
session/root validation between the cleanup calls. A production-function test
covers OLED animation-count and timer cleanup with Local state preserved; runtime
tests cover cleanup ordering and Stop preventing subsequent callbacks. The numeric
removeWorkingAnimation implementation is a no-op, avoiding arbitrary layer removal.
Other retained display layers, exact-screen startup, and hardware integration remain
unfinished. Independent negotiation stays disabled.

### Clear retained Remote OLED console overlays

OLED startup now clears the Remote console's active item count and bounds, cancels
its timer, and marks the frame changed before opening the root. Inactive console
storage is retained but cannot be composited without new items. Production-function
coverage verifies console and timer cleanup with Local state preserved; a runtime
test verifies reset-before-open. Other display-layer state, exact-screen startup,
and hardware validation remain unfinished. Independent negotiation stays disabled.

### Discard retained numeric layers before Remote startup

Remote startup now resets numeric display layers before working-animation and
popup cleanup. The SevenSegment reset cancels the active display timer, destroys
all retained layers without resuming their callbacks, clears popup/transition state,
and invalidates cached digits. It preserves the Local display bank and lowercase
preference. Startup revalidates host/session/song/root ownership after the reset.
Production-function tests cover destruction, idempotent cleanup, blank cached
output, and Local isolation; runtime tests cover ordering and teardown during
cleanup. Full display/UI hardware integration, exact-screen startup, and other
per-view reconnect state remain unfinished. Independent negotiation stays disabled.

### Clear retained OLED scrolling and blinking before Remote startup

The startup layer-reset hook now also clears the Remote OLED main image through
its existing clearMainImage path. This stops retained blink/scroll state, releases
scroller text, cancels the shared animation timer, clears old pixels and permanent
popup bookkeeping, and marks the new canvas dirty before root opening and snapshot
timer servicing. Local OLED state is preserved. Production-function tests cover
cleanup and repeated reset; a runtime test verifies cleanup before root opening
and snapshot timers. Exact-screen startup, remaining per-view reconnect state,
and hardware validation are still unfinished. Independent negotiation stays disabled.

### Clear retained indicator blink records before Remote startup

Startup now clears the Remote indicator blink records as well as both LED blink,
knob blink, and meter-holdoff timers before opening the root. Cancelling timers
alone left old LEDs eligible for reactivation by a new blink of the same type and
left knobs reporting a retained blink. The reset emits no panel output and keeps
the indicator frame and Local state intact. Tests exercise the production indicator
implementation, repeated cleanup, a fresh blink after cleanup, timer isolation,
and runtime reset-before-open ordering. Independent negotiation remains disabled;
other per-view reconnect state and end-to-end validation remain unfinished.

### Reset retained pad transitions before Remote startup

Startup now waits for the Remote pad rendering lock, then cancels its matrix timer
and clears retained transition targets, animation selectors, greyout, tick positions,
and pending output flags before reopening SessionView. Image/occupancy buffers and
cursor preferences are retained for the subsequent full render; no pad output is
sent by cleanup. Production-function tests use real PadState storage to check
Remote cleanup, idempotence, retained image/preferences, and Local isolation. Runtime
coverage verifies retry after a Remote rendering lock and independence from a Local
rendering lock. Exact-screen startup, other per-view reconnect state, and hardware
validation remain unfinished; independent negotiation stays disabled.

### Clear the retained Remote button bank at startup

Remote startup now resets the complete Buttons state bank, replacing the narrower
sticky-Shift cleanup. Pressed flags, record/select release bookkeeping, cross-screen
release intent, sticky-Shift release eligibility, and press timestamps are cleared
without dispatching release commands. Shift change notification remains set. Tests
compile the production reset against real button banks and verify repeated cleanup
and Local isolation; runtime coverage verifies reset-before-open without injected
input. Independent negotiation stays disabled. Remaining per-view reconnect state,
exact-screen startup, and end-to-end hardware validation remain unfinished.

### Clear retained MIDI-learn targets before Remote root opening

SessionView startup reset now clears the active View's MIDI-learn target pointers,
selected learn action, delete-on-release intent, flash state, and flash timer. It
avoids completion hooks and target dereferences, and preserves any pending settings
save flag. Production-function tests check target and session isolation, timer ownership,
repeated cleanup, and preservation of edited data/save intent; the SessionView reset
test verifies integration into startup. Other per-view reconnect state, exact-screen
startup, and hardware validation remain unfinished. Independent negotiation stays disabled.

### Clear retained modulation targets before Remote root rendering

SessionView startup now clears the active View's modulation model-stack storage
and scratch extension before root opening can redraw. It also resets the edited
region, pending automation-indicator refresh, and deferred mod-encoder popup flag
and timer. Opening then binds the current song through the normal path. Cleanup
performs no target dereferences or MIDI feedback. Production-function tests poison
real ModelStack storage and verify Remote cleanup, Local isolation, and idempotence;
SessionView reset coverage verifies the startup hook. Other per-view state,
exact-screen startup, and hardware validation remain unfinished. Independent
negotiation stays disabled.

### Drop retained SoundEditor context before Remote root opening

SessionView startup now resets the session's SoundEditor model pointers and menu
navigation record without calling detached menus' completion hooks. It clears
source/range selection, shortcut toggles and blink state, note-row selection, and
cached pad-render readiness while retaining the shortcut-map preference. Tests
compile the production reset and verify Remote cleanup, repeated reset, Local
isolation, and integration into SessionView startup. Per-menu internal state still
requires auditing; this does not claim all menu lifetimes are safe. Exact-screen
startup and hardware validation remain unfinished. Independent negotiation stays disabled.

### Isolate horizontal-menu navigation debounce between sessions

Horizontal menu navigation now keeps its button debounce timestamp per UI session.
A Local button press no longer suppresses Remote navigation within 140 ms, and vice
versa. The timestamp remains shared across menus within each session, preserving
same-device debounce behavior. Production-handler tests cover simultaneous selection,
page and shifted-chain navigation, debounce across menu instances, and release/
unrelated-button fallthrough. Independent negotiation remains disabled; further menu
state auditing, exact-screen startup, and hardware validation remain unfinished.

### Isolate recording-follow scroll restoration between sessions

The saved horizontal scroll position used during auto-extending recording is now
session-owned. SessionView initialization, focus changes, and encoder navigation
on one device no longer clear the other device's saved return position. Clip and
song views still share that position within the same session. Production-function
tests cover separate positions, same-session sharing, focus cancellation, and
encoder cancellation with parent delegation preserved. Independent negotiation
remains disabled; further UI auditing, exact-screen startup, and hardware validation
remain unfinished.

### Isolate decimal cursor-render state between sessions

Decimal and patch-cable-strength rendering now read cursor-motion state from the
active UI session instead of a shared global flag. The decimal encoder handler
restores the previous value after rendering, so nested rendering in the same session
does not clear an outer edit's cursor state. Production-handler tests cover nested
Remote/Local rendering, same-session nesting, and unchanged numeric-display digit
movement. Independent negotiation remains disabled; remaining UI auditing,
exact-screen startup, and hardware validation are unfinished.

### Keep playhead scratch buffers local to each render call

Keyboard recording cursors and session launch playheads now build tick-square
arrays on the stack rather than in shared mutable globals. A nested render cannot
overwrite an outer call's pending tick positions. Production-function tests suspend
the tick consumer, render the other session, then verify both positions and recording
colours, plus suppressed/out-of-range output. This exercises reentrant buffer
ownership with a stub consumer, not hardware interrupt timing. Independent negotiation
stays disabled; remaining UI auditing, exact-screen startup, and hardware validation
are unfinished.

### Route browser catalog limits and search direction through the active session

Browser file lists were already session-owned, but the catalog size limit and search
direction remained shared globals. They now live alongside the active session's
file list. Interleaved browsing therefore keeps each session's requested search side,
culling budget, deletion counters, and retained filename boundaries together.
Regression tests compile the production state accessors, sorting, and culling methods
and exercise opposite search directions and unequal budgets across Local and Remote.
The file-container collaborator is a native fixture; filesystem timing and hardware
reentrancy still require validation. Independent negotiation remains disabled pending
remaining UI auditing, exact-screen startup, and hardware validation.

### Isolate DX cartridge browsing state

The shared DX cartridge menu now keeps the loaded cartridge, patch selection, and
scroll position in separate Local/Remote banks. Loading or navigating a cartridge
in one session no longer changes the cartridge used by the other session's menu.
Encoder handling also ignores absent/empty cartridges and uses a valid scroll range
when a cartridge has fewer patches than visible OLED rows. Regression tests compile
the production state accessor, loader entry point, and encoder handler with file,
patch-application, and display collaborators stubbed. They cover interleaved loads,
selection and scroll isolation, short/empty cartridges, and numeric-display clamping.
Actual SD reads and patch application remain outside this fixture. Independent
negotiation remains disabled pending remaining UI auditing, exact-screen startup,
and hardware validation.

### Isolate MIDI device menu viewports

The MIDI device menu now stores its viewport scroll position per UI session, matching
its already session-owned selected value and sound-editor cable target. Opening,
scrolling, or returning to the menu on Remote no longer moves Local's viewport.
Regression tests compile the production entry, scrolling, and OLED-list rendering
methods with a fixture cable list. They cover disconnected device gaps, independent
selection/viewport rendering, return from device settings, and the upper list boundary.
USB hotplug timing remains outside this fixture. Independent negotiation remains
disabled pending the remaining routing/lifetime audit, exact-screen startup, and
hardware validation. A further retained-state audit target is MultiRange's shared
navigation destination and viewport.

### Isolate MultiRange navigation and viewport

MultiRange now retains its destination menu and viewport separately for Local and
Remote. Sound Editor and horizontal-menu entry/back-navigation paths use the active
session's destination accessor; range scrolling and OLED rendering use its viewport.
Production-function regression tests cover interleaved entry with different destinations,
note-driven range selection in both directions, unchanged-note redraw suppression,
edge-edit navigation suppression, and numeric-display behavior. Source/model and
rendering collaborators are stubbed; range allocation, deletion, and full OLED output
are outside this fixture. This resolves the retained-state target identified above.
Independent negotiation stays disabled pending the remaining routing/lifetime audit,
exact-screen startup, and hardware validation.

### Isolate oscillator recorder return destinations

Oscillator recorder menu items now keep their pending horizontal-menu parent and
child destination per UI session. Sound Editor writes those session-owned targets;
recorder entry focuses and consumes only the active session's targets. Production
entry/accessor tests cover interleaved destinations, source and navigation setup,
recorder dispatch, and failed-open navigation/timer recovery, including failure when
the editor is no longer current. UI opening, recording, and timers are fixture
collaborators; actual recording and hardware timing remain outside this test.
Independent negotiation remains disabled pending remaining routing/lifetime work,
exact-screen startup, and hardware validation.

### Isolate patch-cable strength interaction state

Patch-cable strength menus now keep pending polarity, the UI's cable-existence flag,
and the delay before forwarding horizontal encoder movement in separate session
banks. Entry, polarity controls/indicators, and cable-creation lookup use the same
active bank. Regression tests compile production entry, state accessors, horizontal
encoder routing, and numeric-display dot selection. They cover interleaved existing
and absent cable contexts and independent delay expiry. Model/display collaborators
are stubbed; existing parameter-lookup suites also exercise the updated creation
accessors. Independent negotiation remains disabled pending remaining routing/lifetime
work, exact-screen startup, and hardware validation.

### Reject invalid patch-cable edit contexts safely

Patch-cable lookup now rejects missing sound-editor owners and non-SOUND manager
layouts before accessing the cable collection, and tolerates a null collection lookup
result. The strength writer checks both the returned stack and its parameter before
writing or refreshing automation. Production lookup tests cover missing owners,
matching GLOBAL contexts, and null collection results in both diagnostic builds.
Production writer tests cover missing stacks/parameters and successful Local editing
after a rejected Remote edit. These checks prevent dereferencing absent contexts;
they do not establish the lifetime of arbitrary stale pointers. Independent negotiation
remains disabled pending the remaining routing/lifetime audit, exact-screen startup,
and hardware validation.

### Handle rejected parameter-kind lookups

Unpatched parameter menus now return Kind::NONE when lookup yields no stack,
parameter, or collection instead of dereferencing the rejected result. The kind
query lives with the production lookup code so native tests compile the actual
implementation. Patched and unpatched menu lookups also tolerate an absent base
model stack. Regression cases cover sound/global kinds in different sessions,
invalid IDs, missing managers/stacks, and preservation of Local after rejected
Remote queries, with diagnostics both enabled and disabled. This guards absent
contexts, not arbitrary stale-pointer lifetimes. Independent negotiation stays
disabled pending remaining routing/lifetime work, exact-screen startup, and hardware
validation.

### Validate standard and pan parameter reads without allocation

Patched/unpatched standard and pan readers now validate the active parameter manager
layout and parameter ID before accessing stored values. Rejected reads preserve the
session's displayed value. Reads remain direct ParamSet reads, avoiding allocation
of automation parameters merely to redraw a menu. Production-reader tests cover
missing/invalid managers and IDs, Local/Remote isolation, existing scaling, and GLOBAL
support for unpatched parameters while patched readers reject that layout. Manager
and storage collaborators are fixtures; production scaling functions are used.
Independent negotiation remains disabled pending remaining routing/lifetime work,
exact-screen startup, and hardware validation.

### Validate patching indicator contexts

Patched parameter name dots and source-shortcut blink queries now reject absent
managers, non-SOUND layouts, and invalid parameter IDs before accessing patch cables.
Rejected contexts report the existing no-indicator value (255). Production-handler
tests cover rejection without cable access, active-session cable selection, matching
and nonmatching sources, and preservation of the supplied colour. Manager and cable
collections are fixtures; rendering and physical LEDs remain outside this test.
Independent negotiation remains disabled pending remaining routing/lifetime work,
exact-screen startup, and hardware validation.

### Guard patch-strength entry, reads, and polarity writes

Patch-strength menu entry, value reads, and polarity writes now reject absent or
non-SOUND parameter managers before accessing the cable collection. Rejected reads
preserve the session's displayed value, while a valid context with no cable retains
the existing zero-value behavior. Extended production-handler tests cover rejection
without collection access, positive/negative value scaling, polarity changes on the
active manager only, and isolation of the other session's displayed value. Model
collections and UI entry dependencies remain fixture collaborators. Independent
negotiation stays disabled pending remaining routing/lifetime work, exact-screen
startup, and hardware validation.

### Reject incomplete automation-selection contexts

Automation parameter selection now validates the parameter, collection, and relevant
clip/output or song before changing knob indicators or remembered selection. Missing
contexts return without selection side effects. Production-handler regression tests
cover rejected stacks and contexts, valid clip/song selection, and preservation of
Local state during Remote selection. Model/UI collaborators are fixtures; surrounding
root-view transitions and stale-pointer lifetimes remain separate audit work.
Independent negotiation remains disabled pending remaining routing/lifetime work,
exact-screen startup, and hardware validation.

### Gate automation view navigation on successful selection

Automation selection now reports success. Menu button routing only sets the background
view state, swaps roots, and initializes automation after a successful selection;
failed reselection also skips rendering/indicator updates. The arranger-context check
handles an absent song. Production button-handler tests cover rejected entry and
reselection, no-song routing, successful Remote entry/back navigation, and preservation
of the Local root. UI/model collaborators remain fixtures. This completes the immediate
caller handling for the previous selection guard; stale previous-root pointers and
broader lifetime auditing remain separate. Independent negotiation stays disabled
pending remaining routing/lifetime work, exact-screen startup, and hardware validation.

### Validate and consume automation return destinations

Automation menu return paths now share one helper. Missing or self-referencing
previous views are rejected before root, menu-state, or indicator changes; successful
returns consume the saved pointer. The previous-view member is explicitly initialized
to null. Production button/helper tests cover invalid returns via Clip, Back, and
Select, successful Clip/Back returns, and preservation of Local's pending destination
when Remote returns. This does not validate arbitrary stale pointer lifetimes or
provide a fallback destination for corrupted state. Independent negotiation remains
disabled pending remaining routing/lifetime work, exact-screen startup, and hardware
validation.

### Reset retained automation navigation on session startup

Remote SessionView startup now clears the active automation view's saved return
pointer, menu-background flag, and arranger flag. Valid parameter selection explicitly
sets clip/arranger context before indicator and shortcut processing instead of relying
on a previous caller to clear arranger mode. Rejected selections preserve existing
state. Production-method tests cover idempotent Remote reset with Local preservation,
clip selection after retained arranger state, and startup wiring. Other automation
gestures, object lifetime, exact-screen startup, and hardware validation remain separate
work; independent negotiation remains disabled.

### Reset retained automation gestures on remote startup

SessionView startup now clears the active automation view's pad selections (including
both row coordinates), multi-pad/interpolation gesture flags, probability-change flag,
and select-knob timing. Existing reset methods cancel parameter, interpolation, and
pad-selection shortcut timers. The interpolation preference is retained. Production
reset-method tests exercise repeated Remote reset and verify Local gestures/timers
remain intact; startup wiring is covered separately. The timer collaborator is a
session-owned fixture, not a hardware scheduler. Inherited editor gestures and broader
lifetime auditing remain separate work. Independent negotiation stays disabled pending
remaining routing/lifetime work, exact-screen startup, and hardware validation.

### Reset retained note-selection display state

Remote startup now resets selected-note coordinates, note-row blink flags/timers,
and held Session-button macro-sidebar state on the session's instrument view, which also serves automation note editing. The reset uses no current model pointers and preserves Local state.
Production-method tests cover repeat reset and session-specific timer cancellation;
startup wiring verifies the owning instrument view is reset. Timer/view storage collaborators
are native fixtures. Active edit-pad buffers own stolen MPE allocations and are not
cleared by this display-state reset; resource-aware gesture cleanup remains separate
work. Independent negotiation remains disabled pending remaining routing/lifetime
work, exact-screen startup, and hardware validation.

### Make edit-pad ownership release idempotent

Ending an edit-pad press now rejects invalid/inactive slots, avoids counter underflow
and invalid row indexing, and clears each released stolen-MPE record and cache flag.
A repeated release cannot free the same allocation again. Production-method tests
record deallocation calls and verify single release of each buffer, preservation of
other presses on the same row, Local/Remote isolation, and stale row/index handling.
The allocator is a fixture. This hardens release itself; startup still must account for
notes temporarily removed during a move before discarding active gestures. Independent
negotiation remains disabled pending that resource-aware cleanup, broader lifetime
work, exact-screen startup, and hardware validation.

### Tolerate absent undo actions during moved-note placement

Vertical note placement now updates undo scroll bookkeeping only when action
allocation succeeds. It continues placement with the null action already supported
by NoteRow insertion, avoiding a crash after notes have been grabbed for a move.
Production-handler tests cover placement without an action and unchanged bookkeeping
with a valid action; note-row and allocation collaborators are fixtures. This does
not provide rollback for destination-note allocation failures or MPE transfer failures.
Independent negotiation remains disabled pending interrupted-move recovery, broader
lifetime work, exact-screen startup, and hardware validation.

### Retain pending moves when melodic row allocation fails

Failed destination-row creation during melodic note moves now retains the active
press and stolen-MPE cache rather than ending the press and freeing its data. Both
deletion flags are cleared so another placement attempt does not first delete a note
that was never placed. Existing kit-boundary cancellation is unchanged. Production
placement-handler tests cover failed creation followed by successful note insertion,
cache retention, and kit cancellation without row allocation. Storage/MPE collaborators
are fixtures. This preserves retry data, not rollback on release: restoring the source
when the gesture is abandoned, destination insertion failure, and MPE transfer failure
remain unresolved. Independent negotiation stays disabled pending those recovery paths,
broader lifetime work, exact-screen startup, and hardware validation.

### Allocate expression storage before placing cached-MPE notes

Moved notes with cached MPE nodes now require successful destination expression-set
allocation before note insertion. Failure reports insufficient RAM, preserves the
pending press/cache, and clears deletion flags for retry. Empty caches do not trigger
allocation. Production placement tests inject allocation failure, then verify later
note insertion and transfer dispatch after allocation succeeds, plus the empty-cache
case. Note/expression storage are fixtures; individual parameter allocation and node
insertion failures still need recovery, as does abandonment of a pending move.
Independent negotiation remains disabled pending those paths, broader lifetime work,
exact-screen startup, and hardware validation.

### Preallocate required MPE parameters before destination note insertion

Moved-note placement now ensures each expression parameter with cached nodes exists
before inserting the destination note. Any parameter allocation failure reports RAM
exhaustion and retains the pending press/cache with deletion flags cleared. The later
transfer requests existing parameters only. Production-handler tests inject failure
at each MPE dimension, verify no insertion/transfer before all required allocations
succeed, then retry and check transfer dispatch for every cached dimension. Empty
dimensions do not allocate. Parameter/note storage remains a fixture. Node insertion
failure and abandonment/source restoration still require recovery. Independent
negotiation remains disabled pending those paths, broader lifetime work, exact-screen
startup, and hardware validation.

### Preserve destination automation on node replacement allocation failure

AutoParam::insertStolenNodes counts removed and replacement nodes, reserves only
net growth, and then edits the destination with capacity-preserving deletion and
allocation-free insertion. This replaces the temporary full-array clone. Failed
reservation leaves destination nodes intact; equal-size or smaller replacements
reuse existing storage, even when every original node is removed. Growth can still
require the allocator to relocate storage, and smaller replacements retain capacity.

Native lifecycle tests cover allocation-budget failures and retries, wrapped
replacement, automation/interpolation flags, source-record preservation, and growth,
shrink, and clear with allocations disabled. Exhaustive small-ring tests exercise
every valid deletion range and start position, including full deletion and capacity
reuse; invalid ranges leave the array unchanged.

The allocation-failure guarantee covers one parameter's node replacement, not all
MPE dimensions or note placement as a group. Concurrent model lifetime,
abandoned-move restoration, exact-screen startup, and hardware validation remain.
Independent negotiation stays disabled.

### Reserve all cached MPE dimensions before vertical note placement

Vertical held-note placement now reserves the destination node capacity for every
cached MPE dimension before inserting the note or transferring any dimension.
Reservation uses the same net-growth calculation as insertStolenNodes and does not
modify destination node values. If one dimension cannot reserve, the placement
stops with the entire source cache retained and deletion flags cleared for retry.
Empty cached dimensions do not create parameters or reserve storage.

Production-method tests inject reservation failure in each dimension, verify no
note or dimension transfer, and retry successfully. They also cover note insertion
failure after reservation. Native lifecycle tests exercise actual array allocation
failures, unchanged destination nodes and source records, and successful reservation
followed by transfer with allocations disabled.

This closes node-storage reservation failures in vertical held-note placement; it
is not a transaction across allocator callbacks, undo snapshot failures, or model
lifetime changes. Other move paths and abandoned-source restoration still need work.
Independent negotiation remains disabled.

### Capture held-note expression data before source deletion

Vertical held-note grabs now stage MPE copies for every uncached participating
press before removing any source note or expression nodes. Copies preserve the
source and use relative positions, including wrapped regions. Required parameter
undo snapshots are also prepared before deletion. Failed capture or snapshot
preparation frees the staged records, leaves caches uncommitted, and cancels the
grab. Successful preparation transfers ownership into the existing press caches;
subsequent source removal does not allocate another copy.

Production-method tests fail each of six dimension captures across two held notes,
check unchanged sources and cleanup, and retry successfully. They also cover undo
snapshot failure and reuse of an existing cache. Native lifecycle tests exercise
real copy allocation failure, successful wrapped and non-wrapped copies, empty
regions, and rejection of a nonempty output record.

This protects source capture failures. A later note-deletion failure, changes during
allocator callbacks, and abandonment after source removal still need broader move
recovery and lifetime handling. Independent negotiation remains disabled.

### Reject invalidated node captures after allocation callbacks

Node capture now checks song/UI ownership, both structural-refresh revisions,
source node count and first-node address, and output-record ownership after undo
snapshot preparation and after capture allocation, including failed allocation.
External ownership checks run before accessing retained model storage. If a callback
invalidates the capture, the operation returns BUG and releases its new buffer
without copying through stale indices, deleting source nodes, or overwriting a
nested capture's record. Failed undo snapshot preparation also stops capture.

Native lifecycle tests cover source clearing with successful and failed allocation,
both copy and destructive capture, nested output-record publication, either UI's
structural refresh, and parameter release during a structural callback.

These checks do not detect every in-place edit or unannounced lifetime change.
Multi-note deletion transactions, abandoned-move restoration, and broader lifetime
coverage remain; independent negotiation stays disabled.

### Revalidate capture boundaries after in-place node edits

Capture validation now repeats the region searches after allocation or snapshot
preparation. This catches callbacks that move nodes across the region start, end,
or wrapped end without changing the array's address or size. Invalid selection
indices return BUG before copying or deleting nodes. Changes that preserve the
selection use the current node positions and values.

Destructive capture also rejects an already-owned output record before allocation
or source mutation, preventing overwrite and leakage of an earlier capture.
Native lifecycle tests cover each boundary for both copy and destructive capture,
valid within-region edits, and preservation of an existing owned record.

This is selection validation rather than a complete edit transaction. Multi-note
deletion recovery, unannounced lifetime changes, and abandoned-move restoration
remain outstanding; independent negotiation stays disabled.

### Reserve source-note deletion history before removing held notes

Vertical held-note grabs now reserve one small note-existence undo record per
participating press before committing caches or deleting sources. Reservation
failure frees every unused record and cancels the grab while source notes remain
intact. Structural-refresh, song, and UI-owner changes during reservation also
cancel before deletion. This reserves individual consequences rather than copying
whole note arrays.

Note deletion can consume the prepared storage without allocating another undo
record. Unused storage (including when an existing note-array snapshot covers the
edit) remains caller-owned and is released by the grab's staged cleanup. Existing
callers retain normal allocation behavior through default arguments.

Production-method tests fail each of two deletion reservations and verify unchanged
sources, cleanup, and retry; a structural-change test checks cancellation. Undo tests
exercise actual recording/deletion code with allocations disabled after reservation,
missing prepared storage, existing array snapshots, and invalid recording context.

This closes deletion-history allocation failures for this grab path. It does not
provide rollback for every invalid-context error, unannounced lifetime change, or
abandoned move. Independent negotiation remains disabled.

### Validate grab context at every preparation boundary

Held-note grabs now check song, UI owner, and both structural-refresh revisions
immediately after action creation, after each MPE snapshot attempt, and after each
capture, as well as after deletion-history reservation. Snapshot failure and
successful capture both pass through the same context check before the grab can
reuse source pointers or publish caches. This also covers preparation with no
undo action, which previously skipped the reservation-time check entirely.

Production-method tests cover action-creation invalidation with and without an
action, successful and failed snapshot invalidation, capture invalidation without
an action, song replacement, and a temporary peer UI scope that safely returns to
the original owner. Staged copies are freed on cancellation.

Unannounced object/gesture changes and complete abandoned-move recovery still
require further work. Independent negotiation remains disabled.

### Reject changed held-note gestures during grab preparation

Grab preparation now retains a fixed-size metadata snapshot of the press slots.
After each yielding preparation boundary, it checks active state, coordinates,
note attributes, deletion/cache flags, and owned MPE record counts and pointers.
A callback that cancels or changes a press, activates another slot, or replaces a
cache cancels the outer grab without publishing old staged data over the new state.
Empty records do not require reading their uninitialized pointer fields. This adds
bounded stack metadata, not another copy of the cached node arrays.

Production-method tests change fifteen gesture fields/states at each of action
creation, snapshot preparation, capture, and deletion reservation. They check source
preservation, staged cleanup, and retention of callback changes. A separate test
replaces an existing cache pointer while retaining its node count.

Identical release/repress cycles, unannounced model lifetime changes, and full
abandoned-move restoration remain outside this metadata comparison. Independent
negotiation stays disabled.

### Detect identical release/repress cycles during grab preparation

Each edit-pad slot now retains a gesture revision advanced by a completed release.
Grab preparation snapshots and checks that revision even for inactive slots, so a
release/repress cycle cannot evade validation by recreating identical pad, note,
and cache metadata. Duplicate releases remain idempotent and do not advance the
revision again. The counter stays in the owning view/session's press slot.

Production release tests cover repeated slot reuse, duplicate release, and peer
session isolation. Grab tests recreate the same metadata with an advanced revision
at all four preparation boundaries and verify cancellation, staged cleanup, and
preservation of the newer gesture.

Unannounced model lifetime changes and complete abandoned-move restoration remain
outstanding. Independent negotiation stays disabled.

### Stop invalidated destination placement at callback boundaries

Vertical held-note placement now checks song/UI ownership and structural revisions
after action creation, then checks the active press's gesture revision after row
creation, expression-set creation, parameter creation, node reservation, note
insertion, and each expression transfer. It stops before subsequent stale accesses
or gesture-flag writes when those checks fail. The placement accounts for its own
NoteRowVector insertion's two peer-refresh notifications; additional structural
changes still invalidate the operation.

Production-method tests cover structural and gesture invalidation at all four
allocation preparation stages, action-creation invalidation, post-insertion and
post-transfer cancellation, and normal row creation in both UI sessions. Existing
allocation-failure retry tests continue to pass with actual PeerStructuralChange
notifications in the row-creation fixture.

Stopping after insertion or transfer does not roll back completed mutations. Full
transaction recovery, in-place gesture changes without release, and unannounced
model lifetime changes remain. Independent negotiation stays disabled.

### Preserve placement's original gesture set across action creation

Destination placement now captures active state and gesture revision for every
press slot before creating an undo action. It validates the full set after action
creation and validates each original slot before processing it. A release/repress
or newly activated slot cannot be adopted into the older placement operation, even
when the callback occurs while an earlier note is being inserted. Checks after
allocation steps use the original slot revision rather than a later baseline.

Production-method tests cover re-press, cancellation, and activation during action
creation, replacement/activation of a later slot during earlier insertion, and
normal continuation after placement itself cancels a missing kit row.

Earlier completed placements are not rolled back on cancellation. In-place edits
without a release, broader lifetime protection, and abandoned-move restoration
remain outstanding; independent negotiation stays disabled.

### Validate placement metadata without requiring a release

Grab and placement now share edit-pad metadata capture/comparison helpers.
Destination placement retains note attributes, coordinates, flags, and owned cache
counts/pointers alongside gesture identity. Callback edits within the same press
therefore invalidate the old placement, including a cache replacement that keeps
its count. Placement updates only its expected deletion flags after its own note
insertion so normal MPE transfer remains valid. Empty records do not read or compare
unused pointers, and snapshots still copy metadata rather than node arrays.

Production-method tests change position, velocity, cache ownership, and cache count
at action creation, expression creation, parameter creation, node reservation,
note insertion, and expression transfer. They verify cancellation and retention of
the callback's newer state. The existing grab metadata matrix and normal placement
tests exercise the shared helper as well.

Previously completed mutations are not rolled back by these checks. Full move
recovery and unannounced model lifetime changes remain; independent negotiation
stays disabled.

### Check the remaining placement set at each preparation boundary

Placement now validates every not-yet-completed press slot after each callback
boundary, rather than checking only the current slot and delaying validation of
later slots. A change to a later pending gesture during row creation, expression
creation, parameter creation, or node reservation cancels before inserting the
current note. Completed slots are excluded so placement's own earlier flag updates
or missing-kit-row cancellation do not cause false invalidation.

Production-method tests cover later-slot activation, gesture replacement, velocity
changes, and cache replacement at all four preparation stages, checking that
neither note is inserted and newer gesture state is preserved. Existing tests cover
cancellation after insertion and normal continuation after missing kit rows.

Mutations completed before invalidation still require broader transaction recovery;
unannounced model lifetime changes and abandoned moves remain outstanding.
Independent negotiation stays disabled.

### Validate undo-action ownership across move callbacks

Held-note grab and placement now retain the action identity returned by the logger
and require that action to remain the BEFORE history head at every preparation
check. Head-pointer comparison precedes dereferencing the retained action, and an
identity comparison rejects address reuse. An action returned outside the active
history is rejected before recording or placement bookkeeping. No-action paths
continue to work.

Production-method tests replace the action pointer or change its identity during
grab snapshot/capture/reservation and placement expression creation, parameter
creation, node reservation, insertion, and transfer. They check source preservation
before commit, staged cleanup, and stopping subsequent placement work.

These outer checks do not make every callee safe against action destruction while
it is executing, nor do they roll back completed mutations. Broader lifetime and
move recovery work remains; independent negotiation stays disabled.

### Validate note-existence recording's action after allocation

Action::recordNoteExistenceChange now retains action identity, history ownership,
and UI owner before allocating a consequence. After allocation it checks external
history ownership before reading the retained action again, then rejects identity
reuse or a changed UI owner. An invalidated recording frees its new consequence
storage and returns BUG before appending history or treating the error as a safe
allocation-failure rollback.

Production undo tests cover replacement with successful and failed allocation,
action identity reuse, destruction of a registered action in the allocator callback,
and UI-owner changes. They verify preserved notes and no incorrectly appended
consequences. Existing prepared-storage deletion tests remain supported.

Detached actions still rely on caller-owned lifetime. Other recording helpers and
full move rollback require further work; independent negotiation stays disabled.

### Validate note-array recording's action at both allocation boundaries

Note-array snapshot recording now retains action identity, registered history
ownership, and UI owner. It validates them after consequence allocation and again
after vector cloning, checking the history head before accessing the retained
action. Invalidated private consequences are destroyed without being appended to
an obsolete or replacement action.

Production undo tests cover action replacement on successful/failed consequence
allocation and cloning, identity reuse at either boundary, registered-action
destruction during cloning, and UI-owner change. A clone-attempt fixture hook models
callbacks before a failed clone as well as successful ones. Tests check unchanged
source notes, empty histories, and cleanup of allocated consequence storage.

Detached actions still depend on caller-owned lifetime. Other recording helpers and
full move recovery remain outstanding; independent negotiation stays disabled.

### Guard parameter snapshot recording at both allocation boundaries

Parameter undo recording now validates song/UI context, structural revisions,
registered action ownership, and action identity after consequence allocation and
again after snapshot construction. Invalidated calls release private storage and
return false without appending history or writing snapshot-failure state through
an obsolete action. Ordinary allocation or snapshot failure still reports failure
on a valid action and supports retry.

Production Action-method tests inject parameter-snapshot construction outcomes and
callbacks. They cover action replacement, identity reuse, destruction during
construction, structural changes before model-stack use, owner changes, successful
retry, and allocation failure with and without invalidation. The native parameter
lifecycle suite separately covers the real cloning/snapshot implementation.

These outer construction checks do not protect every unannounced target lifetime
change within cloning. Detached actions remain caller-owned; complete move rollback
and remaining lifetime paths still block independent negotiation.

### Preserve nested parameter snapshots during recording

The snapshot-once path now rechecks the consequence list after allocating the
consequence and after cloning automation. A nested edit's published snapshot wins;
the redundant outer allocation is freed, including when that outer allocation or
clone fails. Explicit snapshot requests continue to allow repeated snapshots.
Stealing an already-snapshotted scalar parameter no longer dereferences absent
automation storage.

Extracted production recording tests cover both nesting boundaries, outer failures,
explicit duplicates, source clearing, and scalar parameters. Parameter cloning is
an injected collaborator in these tests. This does not resolve unannounced model
lifetime changes, complete multi-parameter rollback, or hardware validation;
independent mode remains disabled.
