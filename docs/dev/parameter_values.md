...
When cloning through `ParamManager`, destination summary flags are initialized
before `beenCloned()` receives that summary. Parameter sets clear automation and
interpolation flags for entries whose nodes could not be copied. Node-allocation
failure still permits a partial clone; this does not provide transactional rollback.
...
documents how to run the CMake/CTest targets and their coverage limits. The native
lifecycle target now covers real scalar storage, unpatched/expression
manager cloning, consequence undo/redo, and deterministic playback. Its
[coverage notes](../../tests/integration/parameters/lifecycle/README.md) distinguish
those checks, real-object persistence, failure injection, and selected notification
callbacks from untested sound rendering, live recording, and future pool
