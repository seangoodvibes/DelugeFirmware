...
- Fixed CV/Gate channel assignment logic where releasing `BOTH` correctly frees channels `1` and `2` independently (`#4864`).
- Fixed skipped encoder ticks and select-encoder handling issues (`#4842`, `#3963`, `#3964`, `#3936`, `#3938`).
- Fixed nested once-task scheduling re-entry (`#4832`).
- Fixed clip transpose edge cases where empty note rows could incorrectly veto transposition (`#4823`, `#4824`).
- Fixed MIDI real-time playback noise and clock sync stability (`#4822`, `#4770`, `#4769`, `#4618`, `#4462`, `#4352`).
- Fixed USB host / MIDI enumeration reliability issues, including descriptor-request hangs and disappearing devices (`#4836`, `#4610`, `#4535`, `#4759`).
- Fixed browser/file-search behaviour including forward-only search and fast-scroll viewport clamp issues (`#4584`, `#4752`, `#4782`).
- Fixed audible clicks around seamless audio loops, including section launch transitions and first-note playback after song load (`#4739`, `#4858`).
- Fixed memory-safety and null-pointer crash classes across UI/audio paths (`#4805`, `#4638`, `#4598`, `#4596`, `#4476`, `#4475`).

#### Official Firmware Issues

- Folders in file browsers are now marked with a dot on 7SEG displays in the lower-right corner.
- Fixed long-standing memory corruption triggered by waveform rendering / waveform viewing.
- Fixed long-standing arranger playback and automation undo inconsistencies (`#4611`, `#4540`, `#4445`).

#### 1.1 Firmware Issues

- Arpeggiator octave mode (`UP_DOWN`) now loads correctly from song files.

#### 1.2 Firmware Issues

- Pressing back in the DX7 preset browser now brings you back to the DX7 cartridge browser instead of the DX7 source menu.
- Fixed arranger automation view edge cases and related UI-state inconsistencies.
- Fixed patch-cable menu consistency introduced during the 1.2+ UI workflow changes.
- Fixed patch-cable regressions affecting selection, navigation, and automation state (`#4854`, `#4440`).
- Fixed DX7 browser/menu regressions and operator toggle behaviour (`#4866`, `#4401`, `#4669`, `#4101`).
- Fixed linear-recording note-array issues (ghost notes and note-length edge cases) (`#4564`, `#4225`, `#4477`).

#### Pre-1.2 Long-standing Issues (Includes 1.0 / 1.1 Era and Earlier)

- Fixed transpose bounds bugs that could push notes out of playable range (`#2320`, `#4661`, `#4794`).
- Fixed long-standing stuck-note / note-lifecycle issues in keyboard and drone-related flows (`#3168`, `#4660`, `#4119`).
- Fixed a long-standing SampleRecorder lifecycle crash in SD-card routines (`#3309`, `#4671`).
- Fixed looping sample playback regressions and time-stretch crash cases (`#3768`, `#4109`, `#4110`, `#4515`, `#4623`).
- Fixed long-standing patch-cable / randomizer crash paths in specific menu conditions (`#4014`, `#4476`, `#3905`).
