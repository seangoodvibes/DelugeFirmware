...
#### Unclassified Issues
- Fixed something with CV (giving up both means 1 and 2 are each free (#4864)) - mark to elaborate
- Numerous performance improvements relating to task management, voice management, and memory management.
- Fix a number of MIDI issues dealing with device enumeration, sysex data transfer loss
- Fix a number of file browser search issues
- Fix audible click heard when launching section to switch between audio clips which are playing a seamless loop

#### Official Firmware Issues
- Folders in file browsers are now marked with a dot on 7SEG displays in the lower-right corner.
- Fix memory corruption errors caused by viewing the Waveform Renderer

#### 1.1 Firmware Issues
- Arpeggiator octave mode setting is now loaded correctly from song file.

#### 1.2 Firmware Issues

- Pressing back in the DX7 preset browser now brings you back to the DX7 cartridge browser instead of the DX7 source menu.
- Audio clip names are now saved and read from song file.
